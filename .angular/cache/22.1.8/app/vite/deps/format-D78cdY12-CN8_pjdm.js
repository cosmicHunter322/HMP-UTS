...
//#region node_modules/@ionic/core/dist/esm/format-D78cdY12.js
/*!
...
var warnIfValueOutOfBounds = (value, min, max) => {
	...
	for (const val of valueArray) if (min !== void 0 && isBefore(val, min) || max !== void 0 && isAfter(val, max)) {
...
var getHourCycle = (locale, hourCycle) => {
	...
	*/
	if (hourCycle !== void 0) return hourCycle;
	/**
	...
	*/
	...
	if (options.hourCycle !== void 0) return options.hourCycle;
	/**
	...
	*/
	...
	const hour = formatted.formatToParts(date).find((p) => p.type === "hour");
	if (!hour) throw new Error("Hour value not found from DateTimeFormat");
	/**
...
var isMonthFirstLocale = (locale, formatOptions = {
	...
	return new Intl.DateTimeFormat(locale, formatOptions).formatToParts(/* @__PURE__ */ new Date())[0].type === "month";
};
/**
...
var isLocaleDayPeriodRTL = (locale) => {
	return new Intl.DateTimeFormat(locale, { hour: "numeric" }).formatToParts(/* @__PURE__ */ new Date())[0].type === "dayPeriod";
};
...
var convertToArrayOfNumbers = (input) => {
	...
	if (typeof input === "string") processedInput = input.replace(/\[|\]|\s/g, "").split(",");
	let values;
	if (Array.isArray(processedInput)) values = processedInput.map((num) => parseInt(num, 10)).filter(isFinite);
	else values = [processedInput];
	return values;
};
/**
...
var clampDate = (dateParts, minParts, maxParts) => {
	if (minParts && isBefore(dateParts, minParts)) return minParts;
	else if (maxParts && isAfter(dateParts, maxParts)) return maxParts;
	return dateParts;
};
/**
	...
	if (data.year !== void 0) {
		...
		if (data.month !== void 0) {
			...
			if (data.day !== void 0) {
				...
				if (data.hour !== void 0) rtn += `T${twoDigit(data.hour)}:${twoDigit(data.minute)}:00`;
			}
		}
	} else if (data.hour !== void 0) rtn = twoDigit(data.hour) + ":" + twoDigit(data.minute);
	return rtn;
}
/**
...
var getStartOfWeek = (refParts) => {
	...
	if (dayOfWeek === null || dayOfWeek === void 0) throw new Error("No day of week provided");
...
var subtractDays = (refParts, numDays) => {
	...
	if (day === null) throw new Error("No day provided");
	const workingParts = {
		month,
		day,
		year
	...
	*/
	if (workingParts.day < 1) workingParts.month -= 1;
	/**
 ...
 /**
	...
	* 31 + (-5) = 26
	*/
	workingParts.day = getNumDaysInMonth(workingParts.month, workingParts.year) + workingParts.day;
	return workingParts;
};
/**
...
var getPreviousMonth = (refParts) => {
	...
	*/
	...
	return {
		month,
		year,
		day: numDaysInMonth < refParts.day ? numDaysInMonth : refParts.day
	};
};
/**
...
var calculateHourFromAMPM = (currentParts, newAMPM) => {
	...
	*
	*/
	if (currentAMPM === "am" && newAMPM === "pm") newHour = convert12HourTo24Hour(newHour, "pm");
	else if (currentAMPM === "pm" && newAMPM === "am") newHour = Math.abs(newHour - 12);
	return newHour;
};
/**
...
var validateParts = (parts, minParts, maxParts) => {
	...
	*/
	if (day !== null && numDays < day) partsCopy.day = numDays;
	/**
	...
	if (minParts !== void 0 && isSameDay(partsCopy, minParts)) {
		...
		*/
		if (partsCopy.hour !== void 0 && minParts.hour !== void 0) {
			...
			} else if (partsCopy.hour === minParts.hour && partsCopy.minute !== void 0 && minParts.minute !== void 0 && partsCopy.minute < minParts.minute) partsCopy.minute = minParts.minute;
		}
	}
	/**
	...
	if (maxParts !== void 0 && isSameDay(parts, maxParts)) {
		...
		*/
		if (partsCopy.hour !== void 0 && maxParts.hour !== void 0) {
			...
			} else if (partsCopy.hour === maxParts.hour && partsCopy.minute !== void 0 && maxParts.minute !== void 0 && partsCopy.minute > maxParts.minute) partsCopy.minute = maxParts.minute;
		}
	}
	return partsCopy;
};
...
var getClosestValidDate = ({ refParts, monthValues, dayValues, yearValues, hourValues, minuteValues, minParts, maxParts }) => {
	...
	if (yearValues !== void 0) copyParts.year = findClosestValue(year, yearValues.filter((year) => {
		if (minParts !== void 0 && year < minParts.year) return false;
		if (maxParts !== void 0 && year > maxParts.year) return false;
		return true;
	}));
	if (monthValues !== void 0) copyParts.month = findClosestValue(month, monthValues.filter((month) => {
		if (minParts !== void 0 && copyParts.year === minParts.year && month < minParts.month) return false;
		if (maxParts !== void 0 && copyParts.year === maxParts.year && month > maxParts.month) return false;
		return true;
	}));
	if (day !== null && dayValues !== void 0) copyParts.day = findClosestValue(day, dayValues.filter((day) => {
		if (minParts !== void 0 && isBefore({
			...copyParts,
			day
		}, minParts)) return false;
		if (maxParts !== void 0 && isAfter({
			...copyParts,
			day
		}, maxParts)) return false;
		return true;
	}));
	if (hour !== void 0 && hourValues !== void 0) {
		copyParts.hour = findClosestValue(hour, hourValues.filter((hour) => {
			if (minParts?.hour !== void 0 && isSameDay(copyParts, minParts) && hour < minParts.hour) return false;
			if (maxParts?.hour !== void 0 && isSameDay(copyParts, maxParts) && hour > maxParts.hour) return false;
			return true;
		}));
		copyParts.ampm = parseAmPm(copyParts.hour);
	}
	if (minute !== void 0 && minuteValues !== void 0) copyParts.minute = findClosestValue(minute, minuteValues.filter((minute) => {
		if (minParts?.minute !== void 0 && isSameDay(copyParts, minParts) && copyParts.hour === minParts.hour && minute < minParts.minute) return false;
		if (maxParts?.minute !== void 0 && isSameDay(copyParts, maxParts) && copyParts.hour === maxParts.hour && minute > maxParts.minute) return false;
		return true;
	}));
	return copyParts;
};
...
var getLocalizedTime = (locale, refParts, hourCycle, formatOptions = {
	...
	if (timeParts.hour === void 0 || timeParts.minute === void 0) return "Invalid Time";
	return new Intl.DateTimeFormat(locale, {
		...
		*/
		hourCycle
	}).format(/* @__PURE__ */ new Date(convertDataToISO({
		/**
...
var getFormattedHour = (hour, hourCycle) => {
	...
	*/
	if (is24Hour(hourCycle)) return addTimePadding(hour);
...
var getLocalizedDayPeriod = (locale, dayPeriod) => {
	...
	if (localizedDayPeriod) return localizedDayPeriod.value;
...
//#endregion
export { getYear as A, parseMinParts as B, getPartsFromCalendarDay as C, getPreviousYear as D, getPreviousWeek as E, isMonthFirstLocale as F, validateParts as H, isSameDay as I, parseAmPm as L, isAfter as M, isBefore as N, getStartOfWeek as O, isLocaleDayPeriodRTL as P, parseDate as R, getNumDaysInMonth as S, getPreviousMonth as T, warnIfValueOutOfBounds as U, removeDateTzOffset as V, getMonthAndYear as _, convertToArrayOfNumbers as a, getNextWeek as b, getClosestValidDate as c, getFormattedHour as d, getHourCycle as f, getLocalizedTime as g, getLocalizedDayPeriod as h, convertDataToISO as i, is24Hour as j, getTodayLabel as k, getDay as l, getLocalizedDateTime as m, calculateHourFromAMPM as n, formatValue as o, getInternalHourValue as p, clampDate as r, generateDayAriaLabel as s, addTimePadding as t, getEndOfWeek as u, getNextDay as v, getPreviousDay as w, getNextYear as x, getNextMonth as y, parseMaxParts as z };
