import { h as printIonWarning, m as printIonError } from "./index-Dvqtt_WK-CtfmC4By.js";
//#region node_modules/@ionic/core/dist/esm/index-DmnOg_xm.js
/*!
...
var sanitizeDOMString = (untrustedString) => {
	try {
		...
		*/
		blockedTags.forEach((blockedTag) => {
			...
			for (let elementIndex = getElementsToRemove.length - 1; elementIndex >= 0; elementIndex--) {
				...
				else documentFragment.removeChild(element);
				/**
				...
				*/
				...
				for (let childIndex = 0; childIndex < childElements.length; childIndex++) sanitizeElement(childElements[childIndex], domStringAllowedAttributes);
			}
		});
		/**
		...
		*/
		...
		for (let childIndex = 0; childIndex < dfChildren.length; childIndex++) sanitizeElement(dfChildren[childIndex], domStringAllowedAttributes);
...
var sanitizeDOMTree = (root) => {
	if (!isSanitizerEnabled()) return;
	blockedTags.forEach((tag) => {
		const matches = root.querySelectorAll(tag);
		for (let i = matches.length - 1; i >= 0; i--) matches[i].remove();
	});
...
var sanitizeElement = (element, allowedAttributes, allowedAttributePrefixes = []) => {
	...
	*/
	...
	for (let i = element.attributes.length - 1; i >= 0; i--) {
		...
		*/
		if (urlAttributes.includes(lowerName) && normalizedValue.startsWith("data:") && !safeDataImageUri.test(normalizedValue)) element.removeAttribute(attributeName);
	}
	/**
	* Sanitize any nested children
	*/
	const childElements = getElementChildren(element);
	for (let i = 0; i < childElements.length; i++) sanitizeElement(childElements[i], allowedAttributes, allowedAttributePrefixes);
};
/**
...
var isSanitizerEnabled = () => {
	...
	if (config) if (config.get) return config.get("sanitizerEnabled", true);
	else return config.sanitizerEnabled === true || config.sanitizerEnabled === void 0;
	return true;
};
/**
...
var reflectPropertiesToAttributes = (root) => {
	const candidates = [];
	if (root.tagName in elementPropsToReflect) candidates.push(root);
	for (const tagName of Object.keys(elementPropsToReflect)) candidates.push(...Array.from(root.querySelectorAll(tagName.toLowerCase())));
	for (const el of candidates) {
		if (!(el.tagName in elementPropsToReflect)) continue;
		const props = elementPropsToReflect[el.tagName];
		for (const prop of props) {
			const value = el[prop];
			if (typeof value === "string" && value.length > 0 && !el.hasAttribute(prop)) el.setAttribute(prop, value);
		}
	}
};
/**
...
* instead.
*/
var domStringAllowedAttributes = [
	"class",
	"id",
	"href",
	"src",
...
var isAttributeAllowed = (lowerName, allowedAttributes, allowedAttributePrefixes) => {
	if (allowedAttributes.includes(lowerName)) return true;
...
*/
var elementPropsToReflect = { "ION-ICON": [
	"icon",
	"name",
	"src",
	"ios",
...
//#endregion
export { sanitizeDOMTree as a, sanitizeDOMString as i, blockedTags as n, reflectPropertiesToAttributes as r, IonicSafeString as t };
