import { _ as readTask, a as config, c as createEvent, d as getElement, h as printIonWarning, l as forceUpdate, m as printIonError, p as h, r as Host, v as registerInstance, x as writeTask } from "./index-Dvqtt_WK-CtfmC4By.js";
import { t as isRTL } from "./dir-Dojwmvde-JWsS7_L7.js";
import { f as inheritAriaAttributes, i as componentOnReady, r as clamp, u as hasLazyBuild, x as shallowEqualStringMap } from "./helpers-BJFnZngp-COG28xNj.js";
import { c as transition } from "./index-CdoVp_rZ-D9b3Ui-I.js";
import { t as getTimeGivenProgression } from "./cubic-bezier-hHmYLOfE-CQJriwBy.js";
import "./gesture-controller-B_gJaBk0-B8pQAih_.js";
import { i as isPlatform, t as getIonMode } from "./ionic-global-BSaFA7np-C0AYHzhu.js";
import { r as hostContext, t as createColorClasses } from "./theme-byZM6qHV-CVcL0HyM.js";
import { n as attachComponent, r as detachComponent } from "./framework-delegate-CUDbUfWS-SLUux__8.js";
import { T as shouldUseCloseWatcher, p as getOverlaySizeType } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import { a as findIonContent, c as getScrollElement, u as printIonContentErrorMsg } from "./index-obGyiHiO-C73DG7Nj.js";
import "./capacitor-C041I3qZ-AKn9tKTZ.js";
import "./keyboard-Cz4hUjuj-C2WFv-Ft.js";
import { t as createLockController } from "./lock-controller-B-hirT0v-wdq_Ov9y.js";
import { t as createKeyboardController } from "./keyboard-controller-BLvRgxgP-CvdYuwot.js";
//#region node_modules/@ionic/core/dist/esm/ion-app_8.entry.js
/*!
...
var App = class {
	...
	componentDidLoad() {
		this.rIC(async () => {
			...
			if (!config.getBoolean("_testing")) import("./index-DoY4e_3F-WyCxVzn9.js").then((module) => module.startTapClick(config));
			if (config.getBoolean("statusTap", isHybrid)) import("./status-tap-BsTKKfug-DLl4D7l4.js").then((module) => module.startStatusTap());
			if (config.getBoolean("inputShims", needInputShims())) {
				...
				*/
				...
				import("./input-shims-DognYvwv-BLgulV7r.js").then((module) => module.startInputShims(config, platform));
			}
			const hardwareBackButtonModule = await import("./overlays-a_5AhqWk-BAKKNgyR.js").then((n) => n.v).then(function(n) {
				return n.w;
			});
			const supportsHardwareBackButtonEvents = isHybrid || shouldUseCloseWatcher();
			if (config.getBoolean("hardwareBackButton", supportsHardwareBackButtonEvents)) hardwareBackButtonModule.startHardwareBackButton();
			else {
				/**
				* If an app sets hardwareBackButton: false and experimentalCloseWatcher: true
				* then the close watcher will not be used.
				*/
				if (shouldUseCloseWatcher()) printIonWarning("[ion-app] - experimentalCloseWatcher was set to `true`, but hardwareBackButton was set to `false`. Both config options must be `true` for the Close Watcher API to be used.");
				hardwareBackButtonModule.blockHardwareBackButton();
			}
			if (typeof window !== "undefined") import("./keyboard-Dsc85bts-DNYTTm3t.js").then((module) => module.startKeyboardAssist(window));
			import("./focus-visible-BmVRXR1y-BtHv7wfJ.js").then((module) => this.focusVisible = module.startFocusVisible());
		});
	}
	...
	*/
	async setFocus(elements) {
		if (this.focusVisible) this.focusVisible.setFocus(elements);
	}
	rIC(callback) {
		if ("requestIdleCallback" in window) window.requestIdleCallback(callback);
		else this.loadTimeout = setTimeout(callback, 32);
	}
	render() {
		return h(Host, {
...
var needInputShims = () => {
	if (isPlatform(window, "ios") && isPlatform(window, "mobile")) return true;
	if (isPlatform(window, "android") && isPlatform(window, "mobileweb")) return true;
	return false;
};
App.style = appCss();
...
var Content = class {
	...
	disconnectedCallback() {
		...
		if (hasLazyBuild(this.el)) {
			...
			*/
			...
			if (tabsElement !== null && tabsLoadCallback !== void 0) tabsElement.removeEventListener("ionTabBarLoaded", tabsLoadCallback);
			this.tabsElement = null;
	...
	*/
	setupFullscreenResizeObserver() {
		...
		if (this.fullscreenResizeObserver !== void 0) return;
	...
	*/
	readSizeToContent() {
		if (hostContext("ion-popover", this.el)) return true;
		const modal = this.el.closest("ion-modal");
		if (modal === null) return false;
		return getOverlaySizeType(getComputedStyle(modal).getPropertyValue("--height")) === "content";
	}
	resize() {
		/**
		...
		*/
		this.updateSizeToContent();
		if (this.fullscreen) readTask(() => this.readDimensions());
	...
	*/
	...
	readDimensions() {
		...
		if (top !== this.cTop || bottom !== this.cBottom) {
			this.cTop = top;
			this.cBottom = bottom;
			forceUpdate(this);
		}
	...
	onScroll(ev) {
		...
		if (shouldStart) this.onScrollStart();
	...
	*/
	async getScrollElement() {
		...
		*/
		if (!this.scrollEl) await new Promise((resolve) => componentOnReady(this.el, resolve));
	...
	*/
	async getBackgroundElement() {
		if (!this.backgroundContentEl) await new Promise((resolve) => componentOnReady(this.el, resolve));
	...
	*/
	async scrollToPoint(x, y, duration = 0) {
		...
		const step = (timeStamp) => {
			...
			if (deltaY !== 0) el.scrollTop = Math.floor(easedT * deltaY + fromY);
			if (deltaX !== 0) el.scrollLeft = Math.floor(easedT * deltaX + fromX);
			if (easedT < 1) requestAnimationFrame(step);
			else resolve();
		};
	...
	onScrollStart() {
		...
		this.watchDog = setInterval(() => {
			if (this.lastScroll < Date.now() - 120) this.onScrollEnd();
		}, 100);
	}
	onScrollEnd() {
		if (this.watchDog) clearInterval(this.watchDog);
	...
	render() {
		...
		this.resize();
		return h(Host, {
			key: "e1013621096729354ef92915af52dd823e69fdff",
			...
			key: "3d8649b164d6324beaa1bc50e1d204ad06c2ec1a",
			ref: (el) => this.backgroundContentEl = el,
			id: "background-content",
			part: "background"
		}), fixedSlotPlacement === "before" ? h("slot", { name: "fixed" }) : null, h("div", {
			key: "dd2d3bed8690b4f5027a43f5ec60e7e7df18106d",
			class: {
				"inner-scroll": true,
				"scroll-x": scrollX,
				"scroll-y": scrollY,
			...
			part: "scroll"
		}, h("slot", { key: "dc3d4584d9e9afd43699525a619b8e873078fbbf" })), transitionShadow ? h("div", { class: "transition-effect" }, h("div", { class: "transition-cover" }), h("div", { class: "transition-shadow" })) : null, fixedSlotPlacement === "after" ? h("slot", { name: "fixed" }) : null);
	}
	get el() {
		return getElement(this);
	}
	static get watchers() {
		return { "fullscreen": [{ "fullscreenChanged": 0 }] };
	}
};
var getParentElement = (el) => {
	if (el.parentElement) return el.parentElement;
	if (el.parentNode?.host) return el.parentNode.host;
	return null;
};
...
var handleFooterFade = (scrollEl, baseEl) => {
	readTask(() => {
		...
		*/
		...
		const scale = clamp(0, 1 - (scrollTop - (maxScroll - fadeDuration)) / fadeDuration, 1);
		writeTask(() => {
...
var Footer = class {
	...
	async connectedCallback() {
		const promise = createKeyboardController(async (keyboardOpen, waitForResize) => {
			...
			*/
			if (keyboardOpen === false && waitForResize !== void 0) await waitForResize;
	...
	render() {
		...
		const mode = getIonMode(this);
		const tabBar = this.el.closest("ion-tabs")?.querySelector(":scope > ion-tab-bar");
		return h(Host, {
		...
		}), h("slot", { key: "8e63696e7c528d5c38201e546bf08135290d0945" }));
	}
	get el() {
		return getElement(this);
	}
...
var handleContentScroll = (scrollEl, scrollHeaderIndex, contentEl) => {
	readTask(() => {
		...
		if (contentEl.querySelector("ion-refresher.refresher-native") === null) writeTask(() => {
...
var setToolbarBackgroundOpacity = (headerEl, opacity) => {
	...
	*/
	...
	if (opacity === void 0) headerEl.style.removeProperty("--opacity-scale");
	else headerEl.style.setProperty("--opacity-scale", opacity.toString());
};
var handleToolbarBorderIntersection = (ev, mainHeaderIndex, scrollTop) => {
	...
	* fire here as well.
	*/
	const scale = ev[0].intersectionRatio > .9 || scrollTop <= 0 ? 0 : (1 - ev[0].intersectionRatio) * 100 / 75;
...
var handleToolbarIntersection = (ev, mainHeaderIndex, scrollHeaderIndex, scrollEl) => {
	writeTask(() => {
		...
		if (isPageHidden || intersectionArea > 0 && (leftDiff >= 5 || rightDiff >= 5)) return;
		if (event.isIntersecting) {
			setHeaderActive(mainHeaderIndex, false);
			setHeaderActive(scrollHeaderIndex);
		} else if ((intersection.x === 0 && intersection.y === 0 || intersection.width !== 0 && intersection.height !== 0) && scrollTop > 0) {
...
var setHeaderActive = (headerIndex, active = true) => {
	...
	const ionTitles = headerIndex.toolbars.map((toolbar) => toolbar.ionTitleEl);
	if (active) {
		headerEl.setAttribute("role", ROLE_BANNER);
		headerEl.classList.remove("header-collapse-condense-inactive");
		ionTitles.forEach((ionTitle) => {
			if (ionTitle) ionTitle.removeAttribute("aria-hidden");
		});
	} else {
		/**
		...
		*/
		ionTitles.forEach((ionTitle) => {
			if (ionTitle) ionTitle.setAttribute("aria-hidden", "true");
		});
	}
};
var scaleLargeTitles = (toolbars = [], scale = 1, transition = false) => {
...
var handleHeaderFade = (scrollEl, baseEl, condenseHeader) => {
	readTask(() => {
		...
		*/
		...
		const scale = clamp(0, (scrollTop - fadeStart) / 10, 1);
		writeTask(() => {
...
var getRoleType = (isInsideMenu, isCondensed, mode) => {
	if (isInsideMenu) return ROLE_NONE;
	/**
	* Only apply role="none" to `md` mode condensed headers
	* since the large header is never shown.
	*/
	if (isCondensed && mode === "md") return ROLE_NONE;
	return ROLE_BANNER;
};
...
var Header = class {
	...
	async setupCondenseHeader(contentEl, pageEl) {
		...
		*/
		...
		writeTask(() => {
			if (this.collapsibleMainHeader !== void 0) this.collapsibleMainHeader.classList.add("header-collapse-main");
		});
	}
	render() {
		...
		return h(Host, {
			...
			role: getRoleType(hostContext("ion-menu", this.el), isCondensed, mode),
			class: {
				[mode]: true,
				[`header-${mode}`]: true,
		...
		}), h("slot", { key: "b44fab0a9be7920b9650da26117c783e751e1702" }));
	}
	get el() {
		return getElement(this);
	}
...
var RouterOutlet = class {
	...
	async connectedCallback() {
		const onStart = () => {
			...
			if (this.swipeHandler) this.swipeHandler.onStart();
		};
		this.gesture = (await import("./swipe-back-ibFGqXH2-TPpRkj_x.js")).createSwipeBackGesture(this.el, () => !this.gestureOrAnimationInProgress && !!this.swipeHandler && this.swipeHandler.canStart(), () => onStart(), (step) => this.ani?.progressStep(step), (shouldComplete, step, dur) => {
			if (this.ani) {
				this.ani.onFinish(() => {
					this.gestureOrAnimationInProgress = false;
					if (this.swipeHandler) this.swipeHandler.onEnd(shouldComplete);
				...
				*/
				if (!shouldComplete) {
					...
					newStepValue += getTimeGivenProgression([0, 0], [1, 0], [.68, .28], [1, 1], step)[0];
				} else newStepValue += getTimeGivenProgression([0, 0], [.32, .72], [0, 1], [1, 1], step)[0];
				this.ani.progressEnd(shouldComplete ? 1 : 0, newStepValue, dur);
			} else this.gestureOrAnimationInProgress = false;
		});
		if (this.swipeGesture === void 0) this.swipeGesture = config.getBoolean("swipeBackEnabled", this.mode === "ios");
		else this.updateGestureEnabled();
	}
	componentWillLoad() {
		this.ionNavWillLoad.emit();
	}
	updateGestureEnabled() {
		if (this.gesture) this.gesture.enable(this.swipeHandler !== void 0 && this.swipeGesture === true);
	}
	disconnectedCallback() {
		if (this.gesture) {
	...
	async setRouteId(id, params, direction, animation) {
		return {
			changed: await this.setRoot(id, params, {
	...
	async setRoot(component, params, opts) {
		if (this.activeComponent === component && shallowEqualStringMap(params, this.activeParams)) return false;
			...
			progressCallback: opts.progressAnimation ? (ani) => {
				...
				*/
				if (ani !== void 0 && !this.gestureOrAnimationInProgress) {
					...
					ani.onFinish(() => {
						this.gestureOrAnimationInProgress = false;
						if (this.swipeHandler) this.swipeHandler.onEnd(false);
					}, { oneTimeCallback: true });
					/**
...
var ToolbarTitle = class {
	...
	emitStyle() {
		...
		this.ionStyle.emit({ [`title-${size}`]: true });
	}
	getSize() {
	...
	render() {
		...
		const size = this.getSize();
		return h(Host, {
			key: "c5f0b913ff0a22d9bf02da48a313a613c2ac8fe7",
			class: createColorClasses(this.color, {
				[mode]: true,
				[`title-${size}`]: true,
			...
			key: "5253b9e39fb764bfe1e57370719139f8a83e9904",
			class: "toolbar-title"
		}, h("slot", { key: "e38b1eccb0175c36f8cab211aee7ce4d61fc9035" })));
	}
	get el() {
		return getElement(this);
	}
...
var Toolbar = class {
	...
	componentWillLoad() {
		...
		if (firstButtons) firstButtons.classList.add("buttons-first-slot");
		...
		if (lastButtons) lastButtons.classList.add("buttons-last-slot");
	}
	childrenStyle(ev) {
		ev.stopPropagation();
		...
		Object.keys(updatedStyles).forEach((key) => {
			...
			if (newValue !== childStyles[childKey]) hasStyleChange = true;
			if (newValue) newStyles[childKey] = true;
		});
		if (hasStyleChange) {
	...
	render() {
		...
		return h(Host, {
			key: "c16803dc4eae9972ef19bf43aa74b94feb4fd4c1",
			class: {
				...childStyles,
			...
			key: "20e3f85844c94b8399c245d2df125e6ab678d681",
			class: "toolbar-background",
			part: "background"
		}), h("div", {
			key: "12f09e324e8d9130b9d30c493106b500233fb016",
			class: "toolbar-container",
			part: "container"
		}, h("slot", {
			key: "6da0fe99ee0c4d7c969712e2908a4e7e6be5ed2c",
			name: "start"
		}), h("slot", {
			key: "177c346d6dcabb7857d810cda7d25c6dde32e6e2",
			name: "secondary"
		}), h("div", {
			key: "8c921b415940c71bf4c1a2c5c6887f36ad1afd16",
			class: "toolbar-content",
			part: "content"
		}, h("slot", { key: "1f2976b80d967bd51e86ed1357b363271d655df2" })), h("slot", {
			key: "0f7618044a7e8b0d9b715a8e208070f7ef2ee4c8",
			name: "primary"
		}), h("slot", {
			key: "125c393197e0660a3c2638dd4628c41ef7e05f19",
			name: "end"
		})));
	}
	get el() {
