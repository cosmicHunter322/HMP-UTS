...
//#region node_modules/@ionic/core/dist/esm/ion-breadcrumb_2.entry.js
/*!
...
var Breadcrumb = class {
	...
	render() {
		...
		return h(Host, {
			...
			name: "start"
		}), h("slot", { key: "f1cfb934443cd97dc220882c5e3596ea879d66cf" }), h("slot", {
			...
			class: { "breadcrumbs-collapsed-indicator": true }
		}, h("ion-icon", {
			key: "a849e1142a86f06f207cf11662fa2a560ab7fc6a",
			"aria-hidden": "true",
			icon: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='ionicon'><circle cx='256' cy='256' r='48'/><circle cx='416' cy='256' r='48'/><circle cx='96' cy='256' r='48'/></svg>",
			lazy: false
		})), showSeparator && h("span", {
			...
			icon: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='ionicon'><path d='m184 112 144 144-144 144' stroke-linecap='round' stroke-linejoin='round' stroke-width='48px' class='ionicon-fill-none'/></svg>",
			lazy: false,
			"flip-rtl": true
		}) : h("span", null, "/"))));
	}
...
var Breadcrumbs = class {
	constructor(hostRef) {
		...
		*/
		...
		this.resetActiveBreadcrumb = () => {
			const activeBreadcrumb = this.getBreadcrumbs().find((breadcrumb) => breadcrumb.active);
			if (activeBreadcrumb && this.activeChanged) activeBreadcrumb.active = false;
		};
		this.setMaxItems = () => {
			...
			if (maxItems !== void 0 && breadcrumbs.length > maxItems && itemsBeforeCollapse + itemsAfterCollapse <= maxItems) breadcrumbs.forEach((breadcrumb, index) => {
				if (index === itemsBeforeCollapse) breadcrumb.showCollapsedIndicator = true;
				if (index >= itemsBeforeCollapse && index < breadcrumbs.length - itemsAfterCollapse) breadcrumb.collapsed = true;
			});
		};
		this.setBreadcrumbSeparator = () => {
			const { itemsAfterCollapse, itemsBeforeCollapse, maxItems } = this;
			...
			for (const breadcrumb of breadcrumbs) {
				...
				breadcrumb.separator = breadcrumb.separator !== void 0 ? breadcrumb.separator : last ? void 0 : true;
				if (!active && last) {
	...
	onCollapsedClick(ev) {
		const collapsedBreadcrumbs = this.getBreadcrumbs().filter((breadcrumb) => breadcrumb.collapsed);
	...
	static get watchers() {
		return {
			"maxItems": [{ "maxItemsChanged": 0 }],
			"itemsBeforeCollapse": [{ "maxItemsChanged": 0 }],
			"itemsAfterCollapse": [{ "maxItemsChanged": 0 }]
		};
	}
};
Breadcrumbs.style = {
