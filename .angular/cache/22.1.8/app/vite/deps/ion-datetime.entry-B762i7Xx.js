import { a as config, c as createEvent, d as getElement, h as printIonWarning, m as printIonError, p as h, r as Host, v as registerInstance, x as writeTask } from "./index-Dvqtt_WK-CtfmC4By.js";
import { t as isRTL } from "./dir-Dojwmvde-JWsS7_L7.js";
import { b as renderHiddenInput, l as getElementRoot, v as raf } from "./helpers-BJFnZngp-COG28xNj.js";
...
import { n as FOCUS_TRAP_DISABLE_CLASS } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import { startFocusVisible } from "./focus-visible-BmVRXR1y-BtHv7wfJ.js";
import { a as caretUpSharp, c as chevronDown, i as caretDownSharp, s as chevronBack, u as chevronForward } from "./index-BvabGprD-DxcnQO2v.js";
import { A as getYear, B as parseMinParts, C as getPartsFromCalendarDay, D as getPreviousYear, E as getPreviousWeek, F as isMonthFirstLocale, H as validateParts, I as isSameDay, L as parseAmPm, M as isAfter, N as isBefore, O as getStartOfWeek, P as isLocaleDayPeriodRTL, R as parseDate, S as getNumDaysInMonth, T as getPreviousMonth, U as warnIfValueOutOfBounds, V as removeDateTzOffset, _ as getMonthAndYear, a as convertToArrayOfNumbers, b as getNextWeek, c as getClosestValidDate, d as getFormattedHour, f as getHourCycle, g as getLocalizedTime, h as getLocalizedDayPeriod, i as convertDataToISO, j as is24Hour, k as getTodayLabel, l as getDay, m as getLocalizedDateTime, n as calculateHourFromAMPM, o as formatValue, p as getInternalHourValue, r as clampDate, s as generateDayAriaLabel, t as addTimePadding, u as getEndOfWeek, v as getNextDay, w as getPreviousDay, x as getNextYear, y as getNextMonth, z as parseMaxParts } from "./format-D78cdY12-CN8_pjdm.js";
//#region node_modules/@ionic/core/dist/esm/ion-datetime.entry.js
/*!
...
var getDaysOfMonth = (month, year, firstDayOfWeek, showAdjacentDays = false) => {
	...
	let previousNumDays;
	if (month === 1) previousNumDays = getNumDaysInMonth(12, year - 1);
	else previousNumDays = getNumDaysInMonth(month - 1, year);
...
var getHourData = (hourCycle) => {
	switch (hourCycle) {
		case "h11": return hour11;
		case "h12": return hour12;
		case "h23": return hour23;
		case "h24": return hour24;
		default: throw new Error(`Invalid hour cycle "${hourCycle}"`);
	}
};
/**
...
var generateTime = (locale, refParts, hourCycle = "h12", minParts, maxParts, hourValues, minuteValues) => {
	...
	if (hourValues) processedHours = processedHours.filter((hour) => hourValues.includes(hour));
	if (minuteValues) processedMinutes = processedMinutes.filter((minute) => minuteValues.includes(minute));
	if (minParts) {
		/**
			...
			if (minParts.minute !== void 0) {
				...
				*/
				...
				if (minParts.hour !== void 0 && refParts.hour !== void 0) {
					if (refParts.hour > minParts.hour) isPastMinHour = true;
				}
				processedMinutes = processedMinutes.filter((minute) => {
					if (isPastMinHour) return true;
		...
		*/
		if (isSameDay(refParts, maxParts)) {
			...
			*/
			...
			if (maxParts.minute !== void 0 && refParts.hour === maxParts.hour) processedMinutes = processedMinutes.filter((minute) => minute <= maxParts.minute);
...
var generateMonths = (refParts, forcedDate) => {
	...
	*/
	if (forcedDate !== void 0 && (refParts.month !== forcedDate.month || refParts.year !== forcedDate.year)) {
		...
		return isBefore(forced, current) ? [
			forced,
			current,
...
var getMonthColumnData = (locale, refParts, minParts, maxParts, monthValues, formatOptions = { month: "long" }) => {
	const { year } = refParts;
	const months = [];
	if (monthValues !== void 0) {
		let processedMonths = monthValues;
		if (maxParts?.month !== void 0) processedMonths = processedMonths.filter((month) => month <= maxParts.month);
		if (minParts?.month !== void 0) processedMonths = processedMonths.filter((month) => month >= minParts.month);
...
*/
var getDayColumnData = (locale, refParts, minParts, maxParts, dayValues, formatOptions = { day: "numeric" }) => {
	...
	*/
	...
	} else for (let i = minDay; i <= maxDay; i++) {
...
var getYearColumnData = (locale, refParts, minParts, maxParts, yearValues) => {
	...
	if (yearValues !== void 0) {
		...
		if (maxParts?.year !== void 0) processedYears = processedYears.filter((year) => year <= maxParts.year);
		if (minParts?.year !== void 0) processedYears = processedYears.filter((year) => year >= minParts.year);
	} else {
		const { year } = refParts;
		const maxYear = maxParts?.year ?? year;
		const minYear = minParts?.year ?? year - 100;
		for (let i = minYear; i <= maxYear; i++) processedYears.push(i);
	}
...
var getAllMonthsInRange = (currentParts, maxParts) => {
	if (currentParts.month === maxParts.month && currentParts.year === maxParts.year) return [currentParts];
...
var getCombinedDateColumnData = (locale, todayParts, minParts, maxParts, dayValues, monthValues) => {
	...
	*/
	if (monthValues) months = months.filter(({ month }) => monthValues.includes(month));
	/**
...
var getTimeColumnsData = (locale, refParts, hourCycle, minParts, maxParts, allowedHourValues, allowedMinuteValues) => {
	...
	if (am && !use24Hour) dayPeriodItems.push({
		text: getLocalizedDayPeriod(locale, "am"),
		value: "am"
	});
	if (pm && !use24Hour) dayPeriodItems.push({
...
var isYearDisabled = (refYear, minParts, maxParts) => {
	if (minParts && minParts.year > refYear) return true;
	if (maxParts && maxParts.year < refYear) return true;
	return false;
};
/**
...
var isDayDisabled = (refParts, minParts, maxParts, dayValues) => {
	...
	* at is in this array.
	*/
	if (dayValues !== void 0 && !dayValues.includes(refParts.day)) return true;
	/**
	...
	*/
	if (minParts && isBefore(refParts, minParts)) return true;
	/**
	...
	*/
	if (maxParts && isAfter(refParts, maxParts)) return true;
	/**
...
var getCalendarDayState = (locale, refParts, activeParts, todayParts, minParts, maxParts, dayValues) => {
	...
	*/
	const isActive = (Array.isArray(activeParts) ? activeParts : [activeParts]).find((parts) => isSameDay(refParts, parts)) !== void 0;
	...
	*/
	return {
		disabled: isDayDisabled(refParts, minParts, maxParts, dayValues),
		isActive,
		isToday,
...
var isMonthDisabled = (refParts, { minParts, maxParts }) => {
	if (isYearDisabled(refParts.year, minParts, maxParts)) return true;
	if (minParts && isBefore(refParts, minParts) || maxParts && isAfter(refParts, maxParts)) return true;
	return false;
};
/**
...
var warnIfTimeZoneProvided = (el, formatOptions) => {
	if (formatOptions?.date?.timeZone || formatOptions?.date?.timeZoneName || formatOptions?.time?.timeZone || formatOptions?.time?.timeZoneName) printIonWarning("[ion-datetime] - \"timeZone\" and \"timeZoneName\" are not supported in \"formatOptions\".", el);
};
var checkForPresentationFormatMismatch = (el, presentation, formatOptions) => {
	...
	switch (presentation) {
		...
		case "month":
		case "year":
			if (formatOptions.date === void 0) printIonWarning(`[ion-datetime] - The '${presentation}' presentation requires a date object in formatOptions.`, el);
			break;
		case "time":
			if (formatOptions.time === void 0) printIonWarning(`[ion-datetime] - The 'time' presentation requires a time object in formatOptions.`, el);
			break;
		case "date-time":
		case "time-date":
			if (formatOptions.date === void 0 && formatOptions.time === void 0) printIonWarning(`[ion-datetime] - The '${presentation}' presentation requires either a date or time object (or both) in formatOptions.`, el);
			break;
	}
};
...
`, this.el);
		...
		*/
		...
		this.closeParentOverlay = (role) => {
			...
			if (popoverOrModal) popoverOrModal.dismiss(void 0, role);
		};
		this.setWorkingParts = (parts) => {
			this.workingParts = { ...parts };
		};
		this.setActiveParts = (parts, removeDate = false) => {
			...
			*/
			...
			if (multiple) {
				...
				if (removeDate) this.activeParts = activePartsArray.filter((p) => !isSameDay(p, validatedParts));
				else this.activeParts = [...activePartsArray, validatedParts];
			} else this.activeParts = { ...validatedParts };
			if (this.el.querySelector("[slot=\"buttons\"]") !== null || this.showDefaultButtons) return;
			this.confirm();
		};
		this.initializeKeyboardListeners = () => {
			...
			*/
			const checkCalendarBodyFocus = (ev) => {
				...
				* the inner day.
				*/
				if (ev[0].oldValue?.includes("ion-focused") || !calendarBodyRef.classList.contains("ion-focused")) return;
			...
			*/
			calendarBodyRef.addEventListener("keydown", (ev) => {
				...
				*/
				if (isDayDisabled(partsToFocus, this.minParts, this.maxParts)) return;
		...
		this.initializeCalendarListener = () => {
			...
			*/
			...
			const needsiOSRubberBandFix = getIonMode(this) === "ios" && typeof navigator !== "undefined" && navigator.maxTouchPoints > 1;
			/**
			...
			*/
			writeTask(() => {
				...
				const getChangedMonth = (parts) => {
					...
					const month = (isRTL(this.el) ? calendarBodyRef.scrollLeft >= -2 : calendarBodyRef.scrollLeft <= 2) ? startMonth : endMonth;
					/**
					...
					*/
					...
					if (forceRenderDate !== void 0) return {
					...
					*/
					if (month === startMonth) return getPreviousMonth(parts);
					else if (month === endMonth) return getNextMonth(parts);
					else return;
				};
				const updateActiveMonth = () => {
					...
					*/
					writeTask(() => {
						...
						if (this.resolveForceDateScrolling) this.resolveForceDateScrolling();
						const activeEl = this.el.shadowRoot.activeElement;
						if (activeEl && activeEl.classList.contains("calendar-day")) activeEl.closest(".calendar-body")?.focus();
					});
				};
				/**
		...
		*/
		this.destroyInteractionListeners = () => {
			...
			if (destroyCalendarListener !== void 0) destroyCalendarListener();
			if (destroyKeyboardMO !== void 0) destroyKeyboardMO();
		};
		/**
		...
		*/
		...
		this.processValue = (value) => {
			...
			* not true.
			*/
			if (hasValue) warnIfValueOutOfBounds(valueToProcess, minParts, maxParts);
			const targetValue = clampDate(Array.isArray(valueToProcess) ? valueToProcess[valueToProcess.length - 1] : valueToProcess, minParts, maxParts);
			...
			*/
			if (hasValue) if (Array.isArray(valueToProcess)) this.activeParts = [...valueToProcess];
			else this.activeParts = {
				month,
				day,
				year,
				hour,
 ...
 /**
		...
		this.animateToDate = async (targetValue) => {
			...
			*/
			...
			isBefore(targetValue, workingParts) ? this.prevMonth() : this.nextMonth();
	...
	get isGridStyle() {
		...
		return (presentation === "date" || presentation === "date-time" || presentation === "time-date") && !preferWheel;
	}
	yearValuesChanged() {
	...
	*/
	async valueChanged() {
		const { value } = this;
		if (this.hasValue()) this.processValue(value);
		this.emitStyle();
	...
	*/
	async confirm(closeOverlay = false) {
		...
		*/
		if (activeParts !== void 0 || !isCalendarPicker) if (Array.isArray(activeParts) && activeParts.length === 0) if (preferWheel)
 /**
		...
		*/
		...
		else this.setValue(convertDataToISO(activeParts));
		if (closeOverlay) this.closeParentOverlay(CONFIRM_ROLE);
	}
	/**
	...
	componentDidLoad() {
		...
		*/
		const hiddenCallback = (entries) => {
			if (entries[0].isIntersecting) return;
			if (!this.hasBeenIntersecting) return;
			this.hasBeenIntersecting = false;
			this.destroyInteractionListeners();
			/**
	...
	*/
	componentDidRender() {
		...
		*/
		const hasCalendarGrid = !preferWheel && [
			"date-time",
			"time-date",
			"date"
		...
		if (minParts !== void 0 && hasCalendarGrid && calendarBodyRef) {
			...
			*/
			if (workingMonth && forceRenderDate === void 0) calendarBodyRef.scrollLeft = workingMonth.clientWidth * (isRTL(this.el) ? -1 : 1);
		}
	...
	componentWillLoad() {
		...
		if (multiple) {
			if (presentation !== "date") printIonWarning("[ion-datetime] - Multiple date selection is only supported for presentation=\"date\".", el);
			if (preferWheel) printIonWarning("[ion-datetime] - Multiple date selection is not supported with preferWheel=\"true\".", el);
		}
		if (highlightedDates !== void 0) {
			if (presentation !== "date" && presentation !== "date-time" && presentation !== "time-date") printIonWarning("[ion-datetime] - The highlightedDates property is only supported with the date, date-time, and time-date presentations.", el);
			if (preferWheel) printIonWarning("[ion-datetime] - The highlightedDates property is not supported with preferWheel=\"true\".", el);
		}
		if (formatOptions) {
			checkForPresentationFormatMismatch(el, presentation, formatOptions);
			warnIfTimeZoneProvided(el, formatOptions);
	...
	*/
	renderFooter() {
		...
		*/
		...
		if (!(this.el.querySelector("[slot=\"buttons\"]") !== null) && !showDefaultButtons && !showClearButton) return;
	...
	*/
	...
	renderCombinedDatePickerColumn() {
		...
		*/
		...
		const result = getCombinedDateColumnData(locale, todayParts, minParts !== void 0 && isAfter(minParts, monthsToRender[0]) ? minParts : monthsToRender[0], maxParts !== void 0 && isBefore(maxParts, lastMonth) ? maxParts : lastMonth, this.parsedDayValues, this.parsedMonthValues);
		let items = result.items;
		const parts = result.parts;
		if (isDateEnabled) items = items.map((itemObject, index) => {
	...
	renderIndividualDatePickerColumns(forcePresentation) {
		...
		const months = forcePresentation !== "year" && forcePresentation !== "time" ? getMonthColumnData(this.locale, workingParts, this.minParts, this.maxParts, this.parsedMonthValues) : [];
		let days = forcePresentation === "date" ? getDayColumnData(this.locale, workingParts, this.minParts, this.maxParts, this.parsedDayValues) : [];
		if (isDateEnabled) days = days.map((dayObject) => {
		...
		const years = forcePresentation !== "month" && forcePresentation !== "time" ? getYearColumnData(this.locale, this.defaultParts, this.minParts, this.maxParts, this.parsedYearValues) : [];
		/**
	...
	renderMonthPickerColumn(months) {
		...
		return h("ion-picker-column", {
			...
			onIonChange: (ev) => {
				...
				if (!this.multiple) this.setActiveParts({
					...activePart,
					month: ev.detail.value
				});
				ev.stopPropagation();
	...
	renderTimePickerColumns(forcePresentation) {
		if ([
			...
			"month-year",
			"year"
		].includes(forcePresentation)) return [];
		const userHasSelectedDate = this.getActivePart() !== void 0;
	...
	renderDayPeriodPickerColumn(dayPeriodData) {
		const { disabled } = this;
		if (dayPeriodData.length === 0) return [];
		const activePart = this.getActivePartsWithFallback();
		return h("ion-picker-column", {
			part: WHEEL_PART,
			"aria-label": "Select a day period",
			style: isLocaleDayPeriodRTL(this.locale) ? { order: "-1" } : {},
			color: this.color,
			disabled,
	...
	renderWheelView(forcePresentation) {
		const { locale } = this;
		return h("div", { class: { [`wheel-order-${isMonthFirstLocale(locale) ? "month-first" : "year-first"}`]: true } }, this.renderWheelPicker(forcePresentation));
	}
	/**
	* Grid Render Methods
	*/
	renderCalendarHeader(mode) {
		...
		}, h("div", { class: "calendar-action-buttons" }, h("div", { class: "calendar-month-year" }, h("button", {
			class: {
		...
		}, h("span", { id: "toggle-wrapper" }, getMonthAndYear(this.locale, this.workingParts), h("ion-icon", {
			"aria-hidden": "true",
			icon: this.showMonthAndYear ? expandedIcon : collapsedIcon,
			lazy: false,
			flipRtl: true
		})), mode === "md" && h("ion-ripple-effect", null))), h("div", { class: "calendar-next-prev" }, h("ion-buttons", null, h("ion-button", {
	...
	renderMonth(month, year) {
		...
		return h("div", {
			...
			if (showAdjacentDays && isAdjacentDay && day !== null) {
				if (day > 20) if (month === 1) {
					_year = year - 1;
					_month = 12;
				} else _month = month - 1;
				else if (day < 15) if (month === 12) {
					_year = year + 1;
					_month = 1;
				} else _month = month + 1;
			}
			...
			if (!isCalDayDisabled && isDateEnabled !== void 0) try {
				/**
				* The `isDateEnabled` implementation is try-catch wrapped
				* to prevent exceptions in the user's function from
				* interrupting the calendar rendering.
			...
			*/
			if (highlightedDates !== void 0 && !isActive && day !== null && !isAdjacentDay) dateStyle = getHighlightStyles(highlightedDates, dateIsoString, el);
			let dateParts = void 0;
			if (!isCalendarPadding && !isAdjacentDay) dateParts = `calendar-day${isActive ? " active" : ""}${isToday ? " today" : ""}${isCalDayDisabled ? " disabled" : ""}`;
			else if (isAdjacentDay) dateParts = `calendar-day${isCalDayDisabled ? " disabled" : ""}`;
			return h("div", { class: "calendar-day-wrapper" }, h("button", {
				...
				onClick: () => {
					...
					if (isAdjacentDay) {
						...
						if (multiple) this.setActiveParts(referenceParts, isActive);
						else this.setActiveParts({
							...activePart,
							...referenceParts
						});
					}
	...
	renderTimeLabel() {
		if (!(this.el.querySelector("[slot=\"time-label\"]") !== null) && !this.showDefaultTimeLabel) return;
	...
	renderTimeOverlay() {
		...
		return [
			...
			h("button", {
				...
				onClick: async (ev) => {
					...
					if (popoverRef) {
						...
						popoverRef.present(new CustomEvent("ionShadowTarget", { detail: { ionShadowTarget: ev.target } }));
			...
			h("ion-popover", {
				...
				arrow: false,
				onWillPresent: (ev) => {
					ev.target.querySelectorAll("ion-picker-column").forEach((col) => col.scrollActiveItemIntoView());
				},
				style: {
					"--offset-y": "-10px",
	...
	getHeaderSelectedDateText() {
		...
		if (multiple && isArray && activeParts.length !== 1) {
			...
			if (titleSelectedDatesFormatter !== void 0) try {
				headerText = titleSelectedDatesFormatter(convertDataToISO(activeParts));
			} catch (e) {
				printIonError("[ion-datetime] - Exception in provided `titleSelectedDatesFormatter`:", e);
			}
		} else headerText = getLocalizedDateTime(this.locale, this.getActivePartsWithFallback(), formatOptions?.date ?? {
			weekday: "short",
			month: "short",
			day: "numeric"
		});
	...
	renderHeader(showExpandedHeader = true) {
		if (!(this.el.querySelector("[slot=\"title\"]") !== null) && !this.showDefaultTitle) return;
		return h("div", {
			...
			part: "datetime-title"
		}, h("slot", { name: "title" }, "Select Date")), showExpandedHeader && h("div", {
	...
	*/
	renderTime() {
		const { presentation } = this;
		return h("div", { class: "datetime-time" }, presentation === "time" ? this.renderWheelPicker() : this.renderTimeOverlay());
	}
	/**
	...
	*/
	renderDatetime(mode) {
		const { presentation, preferWheel } = this;
		if (preferWheel && (presentation === "date" || presentation === "date-time" || presentation === "time-date")) return [
	...
	render() {
		...
		return h(Host, {
			key: "4e0b529a70924b065bcc6191e08550428b5a52a0",
			...
			key: "cc9d1b92f960984cb0466d0003a240ace67da243",
	...
	static get watchers() {
		return {
			"formatOptions": [{ "formatOptionsChanged": 0 }],
			"disabled": [{ "disabledChanged": 0 }],
			"min": [{ "minChanged": 0 }],
			"max": [{ "maxChanged": 0 }],
			"presentation": [{ "presentationChanged": 0 }],
			"yearValues": [{ "yearValuesChanged": 0 }],
			"monthValues": [{ "monthValuesChanged": 0 }],
			"dayValues": [{ "dayValuesChanged": 0 }],
			"hourValues": [{ "hourValuesChanged": 0 }],
			"minuteValues": [{ "minuteValuesChanged": 0 }],
