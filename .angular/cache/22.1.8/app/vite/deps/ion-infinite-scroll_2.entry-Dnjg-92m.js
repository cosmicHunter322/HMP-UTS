import { _ as readTask, a as config, c as createEvent, d as getElement, p as h, r as Host, v as registerInstance, x as writeTask } from "./index-Dvqtt_WK-CtfmC4By.js";
...
import { c as getScrollElement, i as findClosestIonContent, u as printIonContentErrorMsg } from "./index-obGyiHiO-C73DG7Nj.js";
//#region node_modules/@ionic/core/dist/esm/ion-infinite-scroll_2.entry.js
/*!
...
var InfiniteScroll = class {
	constructor(hostRef) {
		...
		*/
		...
		this.onScroll = () => {
			...
			if (!scrollEl || !this.canStart()) return 1;
			const infiniteHeight = this.el.offsetHeight;
			if (infiniteHeight === 0) return 2;
			const scrollTop = scrollEl.scrollTop;
			const scrollHeight = scrollEl.scrollHeight;
			const height = scrollEl.offsetHeight;
			const threshold = this.thrPc !== 0 ? height * this.thrPc : this.thrPx;
			if ((this.position === "bottom" ? scrollHeight - infiniteHeight - scrollTop - threshold - height : scrollTop - infiniteHeight - threshold) < 0) {
				if (!this.didFire) {
					this.isLoading = true;
					this.didFire = true;
	...
	async connectedCallback() {
		...
		if (this.position === "top") writeTask(() => {
			if (this.scrollEl) this.scrollEl.scrollTop = this.scrollEl.scrollHeight - this.scrollEl.clientHeight;
		});
	}
	...
	*/
	async complete() {
		const scrollEl = this.scrollEl;
		if (!this.isLoading || !scrollEl) return;
		this.isLoading = false;
		if (this.position === "top") {
			...
			*/
			...
			requestAnimationFrame(() => {
				readTask(() => {
					const newScrollTop = scrollEl.scrollHeight - prev;
					requestAnimationFrame(() => {
						writeTask(() => {
	...
	enableScrollEvents(shouldListen) {
		if (this.scrollEl) if (shouldListen) this.scrollEl.addEventListener("scroll", this.onScroll);
		else this.scrollEl.removeEventListener("scroll", this.onScroll);
	}
	render() {
