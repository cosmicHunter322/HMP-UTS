import { c as createEvent, d as getElement, h as printIonWarning, n as Fragment, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import { t as isRTL } from "./dir-Dojwmvde-JWsS7_L7.js";
import { f as inheritAriaAttributes } from "./helpers-BJFnZngp-COG28xNj.js";
...
//#region node_modules/@ionic/core/dist/esm/ion-input-otp.entry.js
/*!
...
var InputOTP = class {
	constructor(hostRef) {
		...
		*/
		this.onBlur = (event) => {
			...
			if (!(relatedTarget != null && inputRefs.includes(relatedTarget))) {
				this.hasFocus = false;
				this.updateTabIndexes();
				this.ionBlur.emit(event);
				if (this.focusedValue !== this.value) this.emitIonChange(event);
			}
		};
		/**
		...
		*/
		this.onKeyDown = (index) => (event) => {
			...
			if (input.selectionStart !== input.selectionEnd || (event.metaKey || event.ctrlKey) && [
				"a",
				"c",
				"v",
				"x",
				...
				"z",
				"y"
			].includes(event.key.toLowerCase())) return;
			if (event.key === "Backspace") {
				if (this.inputValues[index]) {
					for (let i = index; i < length - 1; i++) this.inputValues[i] = this.inputValues[i + 1];
					this.inputValues[length - 1] = "";
					for (let i = 0; i < length; i++) this.inputRefs[i].value = this.inputValues[i] || "";
					this.updateValue(event);
					event.preventDefault();
				} else if (!this.inputValues[index] && index > 0) this.focusPrevious(index);
			} else if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
				this.isKeyboardNavigation = true;
				event.preventDefault();
				const isLeft = event.key === "ArrowLeft";
				if (isLeft && rtl || !isLeft && !rtl) {
					if (this.inputValues[index] && index < length - 1) this.focusNext(index);
				} else this.focusPrevious(index);
		...
		*/
		this.onInput = (index) => (event) => {
			...
			if (value.length - previousValue.length > 1) {
				...
				if (validChars.length === 0) requestAnimationFrame(() => {
			...
			if (this.inputValues[index] && hasAvailableBoxAtEnd && value.length === 2) {
				...
				if (!newChar) newChar = value.split("").find((c, i) => c !== previousValue[i]) || value[value.length - 1];
			...
			const newChar = value[(input.selectionStart ?? value.length) - 1] ?? value[0];
			if (!validKeyPattern.test(newChar)) {
				input.value = this.inputValues[index] || "";
				this.previousInputValues = [...this.inputValues];
				return;
		...
		*/
		this.onPaste = (event) => {
			...
			this.updateValue(event);
			inputRefs[validChars.length < length ? validChars.length : length - 1]?.focus();
		};
	}
	/**
	...
	*/
	processSeparators() {
		...
		let separatorValues;
		if (separators === "all") separatorValues = Array.from({ length: length - 1 }, (_, i) => i + 1);
		else if (Array.isArray(separators)) separatorValues = separators;
		else separatorValues = separators.split(",").map((pos) => parseInt(pos, 10)).filter((pos) => !isNaN(pos));
		if (separatorValues.filter((pos, index) => separatorValues.indexOf(pos) !== index).length > 0) printIonWarning(`[ion-input-otp] - Duplicate separator positions are not allowed. Received: ${separators}`, this.el);
		const invalidSeparators = separatorValues.filter((pos) => pos > length);
		if (invalidSeparators.length > 0) printIonWarning(`[ion-input-otp] - The following separator positions are greater than the input length (${length}): ${invalidSeparators.join(", ")}. These separators will be ignored.`, this.el);
	...
	*/
	initializeValues() {
		...
		if (this.value == null || String(this.value).length === 0) return;
		String(this.value).split("").slice(0, this.length).forEach((char, index) => {
			if (this.validKeyPattern.test(char)) this.inputValues[index] = char;
		});
	...
	*/
	updateValue(event) {
		...
		this.value = newValue;
		this.emitIonInput(event);
		if (newValue.length === length) this.ionComplete.emit({ value: newValue });
	}
	/**
	...
	*/
	focusNext(currentIndex) {
		...
		if (currentIndex < length - 1) inputRefs[currentIndex + 1]?.focus();
	}
	/**
	...
	*/
	focusPrevious(currentIndex) {
		const { inputRefs } = this;
		if (currentIndex > 0) inputRefs[currentIndex - 1]?.focus();
	}
	/**
	...
	*/
	getFirstEmptyIndex() {
		const { inputValues, length } = this;
		return Array.from({ length }, (_, i) => inputValues[i] || "").findIndex((value) => !value || value === "") ?? -1;
	}
	/**
	...
	*/
	updateTabIndexes() {
		...
		for (let i = 0; i < length; i++) if (!inputValues[i] || inputValues[i] === "") {
			firstEmptyIndex = i;
			break;
		}
	...
	*/
	...
	render() {
		...
		return h(Host, {
			key: "bd37661e13cde21feff30c663e6813ce3fff0d9a",
			...
			key: "5e206f870024f1ecfebda999ff31e6e815774deb",
			role: "group",
			"aria-label": "One-time password input",
			class: "input-otp-group",
			...inheritedAttributes
		}, Array.from({ length }).map((_, index) => h(Fragment, null, h("div", { class: "native-wrapper" }, h("input", {
			class: "native-input",
			id: `${inputId}-${index}`,
		...
		})), this.showSeparator(index) && h("div", { class: "input-otp-separator" })))), h("div", {
			key: "c268f838ddab2c8d6a612a11c7d38e6ca076436d",
			class: {
	...
	static get watchers() {
		return {
			...
			"separators": [{ "processSeparators": 0 }],
			"length": [{ "processSeparators": 0 }]
		};
	}
};
var inputIds = 0;
