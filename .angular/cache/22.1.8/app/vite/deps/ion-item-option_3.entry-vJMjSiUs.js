...
import { i as componentOnReady, m as isEndSide } from "./helpers-BJFnZngp-COG28xNj.js";
...
//#region node_modules/@ionic/core/dist/esm/ion-item-option_3.entry.js
/*!
...
var ItemOption = class {
	constructor(hostRef) {
		...
		*/
		this.type = "button";
		this.onClick = (ev) => {
			if (ev.target.closest("ion-item-option")) ev.preventDefault();
		};
	}
	render() {
		...
		return h(Host, {
			key: "a10d36e60d61865141c57c03a114e4845b400a55",
			...
			key: "5cc49ef73318213f6202d1d0f460f629ea65f17f",
			...attrs,
			class: "button-native",
			part: "native",
			disabled
		}, h("span", {
			key: "0643b16660b4ca00b0f3b4dbe2739e9ce076abea",
			class: "button-inner",
			part: "inner"
		}, h("slot", {
			key: "8778d6d09aa9e0db278ce00c5456a1acfe82c15f",
			name: "top"
		}), h("div", {
			key: "4fc849494c8cb2380015161e096b5c61cf517d90",
			class: "horizontal-wrapper",
			part: "container"
		}, h("slot", {
			key: "9d0c0a5a63d8ec636df9c85d48bbdb39fb63af5e",
			name: "start"
		}), h("slot", {
			key: "a40561497c7e7d73173edd24ed335230e6eebb2b",
			name: "icon-only"
		}), h("slot", { key: "b94f2a4aa5bf4a1104497ced1920d6ee43d33659" }), h("slot", {
			key: "91713dbb3e6a617c00f950857f2ed5e5ffceaa9a",
			name: "end"
		})), h("slot", {
			key: "f5af33c3bfb50fc629a06e1120bba55da07b2afb",
			name: "bottom"
		})), mode === "md" && h("ion-ripple-effect", { key: "fc56e1644ef9c5b2e1e66683465a7fd09c05215c" })));
	}
	get el() {
...
var ItemOptions = class {
	...
	render() {
		...
		return h(Host, {
			key: "6e60ff004d48bf7701711e518ec0b21264496ae8",
			class: {
				[mode]: true,
...
var ItemSliding = class {
	...
	disconnectedCallback() {
		...
		if (openSlidingItem === this.el) openSlidingItem = void 0;
	...
	*/
	async open(side) {
		if ((this.item = this.item ?? this.el.querySelector("ion-item")) === null) return;
		...
		*/
		if (side === void 0) side = optionsToOpen === this.leftOptions ? "start" : "end";
	...
	*/
	getOptions(side) {
		if (side === void 0) return this.leftOptions || this.rightOptions;
		else if (side === "start") return this.leftOptions;
		else return this.rightOptions;
	}
	async updateOptions() {
		...
		*/
		...
		let sides = 0;
		this.leftOptions = this.rightOptions = void 0;
		for (const option of options) if ((isEndSide(option.side ?? option.getAttribute("side")) ? "end" : "start") === "start") {
	...
	canStart(gesture) {
		if (document.dir === "rtl" ? window.innerWidth - gesture.startX < 15 : gesture.startX < 15) return false;
		const selected = openSlidingItem;
		if (selected && selected !== this.el) this.closeOpened();
	...
	onStart() {
		...
		*/
		...
		if (contentEl) this.initialContentScrollY = disableContentScrollY(contentEl);
		openSlidingItem = this.el;
		if (this.tmr !== void 0) {
		...
		if (this.item) this.item.style.transition = "none";
	}
	onMove(gesture) {
		if (this.optsDirty) this.calculateOptsWidth();
	...
	onEnd(gesture) {
		...
		if (contentEl) resetContentScrollY(contentEl, initialContentScrollY);
		const velocity = gesture.velocityX;
		let restingPoint = this.openAmount > 0 ? this.optsWidthRightSide : -this.optsWidthLeftSide;
		if (swipeShouldReset(this.openAmount > 0 === !(velocity < 0), Math.abs(velocity) > .3, Math.abs(this.openAmount) < Math.abs(restingPoint / 2))) restingPoint = 0;
		const state = this.state;
		this.setOpenAmount(restingPoint, true);
		if ((state & 32) !== 0 && this.rightOptions) this.rightOptions.fireSwipeEvent();
		else if ((state & 64) !== 0 && this.leftOptions) this.leftOptions.fireSwipeEvent();
	}
	calculateOptsWidth() {
		this.optsWidthRightSide = 0;
	...
	setOpenAmount(openAmount, isFinal) {
		...
		if (isFinal) style.transition = "";
		if (openAmount > 0) this.state = openAmount >= this.optsWidthRightSide + SWIPE_MARGIN ? 40 : 8;
		else if (openAmount < 0) this.state = openAmount <= -this.optsWidthLeftSide - SWIPE_MARGIN ? 80 : 16;
		else {
			/**
			...
			*/
			if (this.gesture) this.gesture.enable(false);
			this.tmr = setTimeout(() => {
				this.state = 2;
				this.tmr = void 0;
				if (this.gesture) this.gesture.enable(!this.disabled);
	...
	getSlidingRatioSync() {
		if (this.openAmount > 0) return this.openAmount / this.optsWidthRightSide;
		else if (this.openAmount < 0) return this.openAmount / this.optsWidthLeftSide;
		else return 0;
	}
	render() {
		return h(Host, {
			key: "02174ca963753d3a51a4249068f3d436f8acb03d",
			class: {
