...
import { p as inheritAttributes, v as raf } from "./helpers-BJFnZngp-COG28xNj.js";
import { t as getIonMode } from "./ionic-global-BSaFA7np-C0AYHzhu.js";
import { i as openURL, r as hostContext, t as createColorClasses } from "./theme-byZM6qHV-CVcL0HyM.js";
import { u as chevronForward } from "./index-BvabGprD-DxcnQO2v.js";
//#region node_modules/@ionic/core/dist/esm/ion-item_8.entry.js
/*!
...
var Item = class {
	...
	labelColorChanged(ev) {
		const { color } = this;
		if (color === void 0) this.labelColorStyles = ev.detail;
	}
	itemStyle(ev) {
		ev.stopPropagation();
		...
		Object.keys(updatedStyles).forEach((key) => {
			if (updatedStyles[key]) {
				...
				if (!childStyles[itemKey]) hasStyleChange = true;
				newStyles[itemKey] = true;
			}
		});
		if (!hasStyleChange && Object.keys(newStyles).length !== Object.keys(childStyles).length) hasStyleChange = true;
		if (hasStyleChange) {
	...
	totalNestedInputs() {
		return {
			covers: this.el.querySelectorAll("ion-checkbox, ion-datetime, ion-select, ion-radio"),
			inputs: this.el.querySelectorAll("ion-input, ion-range, ion-searchbar, ion-segment, ion-textarea, ion-toggle"),
			clickables: this.el.querySelectorAll("ion-router-link, ion-button, a, button")
		};
	}
	setMultipleInputs() {
	...
	*/
	watchForIndicatorControls() {
		...
		this.indicatorControlObserver = new MutationObserver((records) => {
			if (records.some(touchesIndicatorControl)) this.updateInteractivityOnSlotChange();
		});
	...
	setHasSlottedIndicatorControl() {
		...
		this.hasSlottedIndicatorControl = Array.from(controls).some((control) => {
			let slotted = control;
			while (slotted !== null && slotted.parentElement !== this.el) slotted = slotted.parentElement;
	...
	hasStartEl() {
		if (this.el.querySelector("[slot=\"start\"]") !== null) this.el.classList.add("item-has-start-slot");
	}
	getFirstInteractive() {
	...
	render() {
		...
		if (clickable || firstInteractive !== void 0 && !multipleInputs) clickFn = { onClick: (ev) => {
			if (clickable) openURL(href, ev, routerDirection, routerAnimation);
			if (firstInteractive !== void 0 && !multipleInputs) {
				...
				if (ev.isTrusted) {
					if (this.el.shadowRoot.contains(target)) {
						...
						*/
						if (firstInteractive.tagName === "ION-INPUT" || firstInteractive.tagName === "ION-TEXTAREA") firstInteractive.setFocus();
		...
		*/
		...
		return h(Host, {
			...
			part: "detail-icon",
			"aria-hidden": "true",
			"flip-rtl": detailIcon === "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='ionicon'><path d='m184 112 144 144-144 144' stroke-linecap='round' stroke-linejoin='round' stroke-width='48px' class='ionicon-fill-none'/></svg>"
		})), canActivate && mode === "md" && h("ion-ripple-effect", { key: "e5e151bafedd38feb8f71a387a0059aa1e9e90de" })));
	}
	get el() {
...
var ItemDivider = class {
	...
	render() {
		const mode = getIonMode(this);
		return h(Host, {
			key: "b191727cc26c600287fd856e49216bf1ae1d5aad",
			...
			key: "a8e2eb48ca858b64da8ad018fa8d1667030b7143",
			name: "start"
		}), h("div", {
			key: "3219e0b76ac4a96db50a67f1f2794a235111c613",
			class: "item-divider-inner",
			part: "inner"
		}, h("div", {
			key: "31a64775153cd8704458e0e2a4a512096585deb4",
			class: "item-divider-wrapper",
			part: "container"
		}, h("slot", { key: "d46dfb578028790eb6ccaac5e6009a5d96aa831a" })), h("slot", {
			key: "a5fc7ac27e418ea82d9dcc913a8dae7e35493760",
			name: "end"
		})));
	}
	get el() {
...
var ItemGroup = class {
	...
	render() {
		const mode = getIonMode(this);
		return h(Host, {
			key: "d24ee9c70a99d11b7d95dfb4a8066670bdfcbc1d",
			role: "group",
			class: {
				[mode]: true,
...
var Label = class {
	...
	componentDidLoad() {
		if (this.noAnimate) this.loadTimeout = setTimeout(() => {
			this.noAnimate = false;
		}, 1e3);
	}
	disconnectedCallback() {
	...
	render() {
		...
		const mode = getIonMode(this);
		return h(Host, {
			key: "461604c1887f322dc13d7df3d5cdbb7ee1361117",
			class: createColorClasses(this.color, {
				[mode]: true,
...
var List = class {
	...
	*/
	async closeSlidingItems() {
		...
		if (item?.closeOpened) return item.closeOpened();
		return false;
	}
	render() {
		const mode = getIonMode(this);
		...
		return h(Host, {
			key: "b21e063d568b07331ea9443647b3faf33265256a",
			role: "list",
			class: {
				[mode]: true,
				[`list-${mode}`]: true,
...
var ListHeader = class {
	...
	render() {
		...
		const mode = getIonMode(this);
		return h(Host, {
			key: "92ab17533e169b1a1c990285449e0546aac98021",
			class: createColorClasses(this.color, {
				[mode]: true,
				[`list-header-lines-${lines}`]: lines !== void 0
			})
		}, h("div", {
			key: "26639255577380a8e70b61f753aa9ed57590694f",
			class: "list-header-inner",
			part: "inner"
		}, h("slot", { key: "f69055e7a7a47ae8dcbbe289e18b6630f0792714" })));
	}
};
ListHeader.style = {
...
var Note = class {
	...
	render() {
		const mode = getIonMode(this);
		return h(Host, {
			key: "ffe9b4de8bbd89b1ef427f6cd5dbcd354d715e6b",
...
var SkeletonText = class {
	...
	render() {
		...
		return h(Host, {
			key: "1114e5caf6419e9458db8256480b39e6989963b2",
			class: {
				[getIonMode(this)]: true,
				"skeleton-text-animated": animated,
				"in-media": inMedia
			}
		}, h("span", { key: "9137cf837f163fc0653437fc003666cddac103b0" }, "\xA0"));
	}
	get el() {
		return getElement(this);
	}
