import { a as config, c as createEvent, d as getElement, m as printIonError, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import { f as inheritAriaAttributes, m as isEndSide, n as assert, r as clamp } from "./helpers-BJFnZngp-COG28xNj.js";
import "./animation-CAzXJ2I1-D34TSm8w.js";
import { t as getTimeGivenProgression } from "./cubic-bezier-hHmYLOfE-CQJriwBy.js";
import { n as GESTURE_CONTROLLER } from "./gesture-controller-B_gJaBk0-B8pQAih_.js";
...
import { T as shouldUseCloseWatcher, d as focusFirstDescendant, f as focusLastDescendant, m as getPresentedOverlay, r as GESTURE, t as BACKDROP } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import { t as menuController } from "./index-iurdcGHg-CPNbFdxk.js";
import { b as menuSharp, y as menuOutline } from "./index-BvabGprD-DxcnQO2v.js";
//#region node_modules/@ionic/core/dist/esm/ion-menu_3.entry.js
/*!
...
var Menu = class {
		...
		*/
		...
		this.handleFocus = (ev) => {
			...
			*/
			...
			if (lastOverlay && !lastOverlay.contains(this.el)) return;
	...
	typeChanged(type, oldType) {
		...
		if (contentEl) {
			if (oldType !== void 0) contentEl.classList.remove(`menu-content-${oldType}`);
			contentEl.classList.add(`menu-content-${type}`);
			contentEl.removeAttribute("style");
		}
		if (this.menuInnerEl) this.menuInnerEl.removeAttribute("style");
	...
	async connectedCallback() {
		if (typeof customElements !== "undefined" && customElements != null) await customElements.whenDefined("ion-menu");
		if (this.type === void 0) this.type = config.get("menuType", "overlay");
		...
		if (this.el.contains(content)) printIonError(`[ion-menu] - The "contentId" should refer to the main view's ion-content, not the ion-content inside of the ion-menu.`);
		...
		this.menuChanged();
		this.gesture = (await import("./index-BmLuEdV7-B8ieQZPS.js").then((n) => n.n)).createGesture({
			el: document,
	...
	async componentDidLoad() {
		...
		*/
		...
		if (splitPane !== null) this.isPaneVisible = await splitPane.isVisible();
		this.menuChanged();
		this.updateState();
	}
	menuChanged() {
		...
		*/
		if (this.didLoad) this.ionMenuChange.emit({
			disabled: this.disabled,
			open: this._isOpen
		});
	}
	...
	onBackdropClick(ev) {
		if (this._isOpen && this.lastOnEnd < ev.timeStamp - 100) {
			if (ev.composedPath ? !ev.composedPath().includes(this.menuInnerEl) : false) {
				ev.preventDefault();
				ev.stopPropagation();
	...
	onKeydown(ev) {
		if (ev.key === "Escape") this.close(void 0, BACKDROP);
	}
	/**
	...
	*/
	...
	trapKeyboardFocus(ev, doc) {
		...
		*/
		if (this.el.contains(target)) this.lastFocus = target;
		else {
			/**
			...
			* last descendant.
			*/
			if (this.lastFocus === doc.activeElement) focusLastDescendant(el);
		}
	}
	async _setOpen(shouldOpen, animated = true, role) {
		if (!this._isActive() || this.isAnimating || shouldOpen === this._isOpen) return false;
	...
	async loadAnimation() {
		...
		*/
		...
		if (width === this.width && this.animation !== void 0 && isEndSide$1 === this.isEndSide) return;
		this.width = width;
		...
		if (!config.getBoolean("animated", true)) animation.duration(0);
		animation.fill("both");
	}
	async startAnimation(shouldOpen, animated) {
		...
		if (animated) await ani.play();
		else ani.play({ sync: true });
		/**
		...
		*/
		if (ani.getDirection() === "reverse") ani.direction("normal");
	}
	_isActive() {
	...
	canStart(detail) {
		if (!!document.querySelector("ion-modal.show-modal") || !this.canSwipe()) return false;
		if (this._isOpen) return true;
		else if (menuController._getOpenSync()) return false;
	...
	onMove(detail) {
		...
		const stepValue = computeDelta(detail.deltaX, this._isOpen, this.isEndSide) / this.width;
	...
	onEnd(detail) {
		...
		const z = width / 2;
		const shouldCompleteRight = velocity >= 0 && (velocity > .2 || detail.deltaX > z);
		const shouldCompleteLeft = velocity <= 0 && (velocity < -.2 || detail.deltaX < -z);
		...
		if (isOpen && !shouldComplete) shouldOpen = true;
		...
		*/
		newStepValue += getTimeGivenProgression([0, 0], [.4, 0], [.6, 1], [1, 1], clamp(0, stepValue < 0 ? .01 : stepValue, .9999))[0] || 0;
	...
	beforeAnimation(shouldOpen, role) {
		...
		*/
		if (isPlatform("android")) this.el.setAttribute("aria-hidden", "true");
		...
		*/
		...
		if (this.backdropEl) this.backdropEl.classList.add(SHOW_BACKDROP);
		if (this.contentEl) {
		...
		if (shouldOpen) this.ionWillOpen.emit();
		else this.ionWillClose.emit({ role });
	}
	afterAnimation(isOpen, role) {
		this._isOpen = isOpen;
		this.isAnimating = false;
		if (!this._isOpen) this.blocker.unblock();
		if (isOpen) {
			/**
			* When the menu is presented on an Android device, TalkBack's focus rings
			* may appear in the wrong position due to the transition (specifically
			...
			* is shown again.
			*/
			if (isPlatform("android")) this.el.removeAttribute("aria-hidden");
			this.ionDidOpen.emit();
			if (document.activeElement?.closest("ion-menu") !== this.el) this.el.focus();
			...
			*/
			...
			if (this.backdropEl) this.backdropEl.classList.remove(SHOW_BACKDROP);
	...
	updateState() {
		...
		if (this.gesture) this.gesture.enable(isActive && this.swipeGesture);
		/**
		...
		*/
		if (!isActive) {
			...
			*/
			if (this.isAnimating) this.operationCancelled = true;
			/**
	...
	render() {
		...
		*/
		return h(Host, {
			key: "1208ba437fa58680325408d3d4c8580950da1d68",
			...
			key: "c0ddc957b7a7956b41901d77ae02d91e73071c55",
			class: "menu-inner",
			part: "container",
			ref: (el) => this.menuInnerEl = el
		}, h("slot", { key: "6e44c56116431a024f114ab85ded1b822c942fac" })), h("ion-backdrop", {
			key: "5f4bbcf84444d96ec6b02bf567e1aaf95f59078d",
...
var computeDelta = (deltaX, isOpen, isEndSide) => {
	return Math.max(0, isOpen !== isEndSide ? -deltaX : deltaX);
};
var checkEdgeSide = (win, posX, isEndSide, maxEdgeStart) => {
	if (isEndSide) return posX >= win.innerWidth - maxEdgeStart;
	else return posX <= maxEdgeStart;
};
...
var MenuButton = class {
	...
	render() {
		...
		return h(Host, {
			key: "b8869377822b082c6c4d3f16135b50b733eabf65",
			...
			key: "8c712f28c5afd345e403a9afc525931112852708",
			class: "button-inner"
		}, h("slot", { key: "dd872325e42baf435188478c8d1cfc416075089d" }, h("ion-icon", {
			key: "a4f02bdb715b07792c17e12c7837873d70171f88",
			part: "icon",
			icon: menuIcon,
			mode,
			lazy: false,
			...
			key: "564e2a8a804e14d1135502f7b17d8b7b9862e81b",
			type: "unbounded"
		})));
	}
	get el() {
...
var MenuToggle = class {
	...
	render() {
		...
		const hidden = this.autoHide && !this.visible;
		return h(Host, {
			key: "76d21ea6493b8161860c941e3754fef1e20395cc",
			onClick: this.onClick,
			"aria-hidden": hidden ? "true" : null,
			class: {
				[mode]: true,
