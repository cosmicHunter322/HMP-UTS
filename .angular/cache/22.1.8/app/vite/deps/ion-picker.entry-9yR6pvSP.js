...
import { l as getElementRoot } from "./helpers-BJFnZngp-COG28xNj.js";
//#region node_modules/@ionic/core/dist/esm/ion-picker.entry.js
/*!
...
var Picker = class {
	constructor(hostRef) {
		...
		*/
		this.onFocusOut = (ev) => {
			...
			if (!relatedTarget || relatedTarget.tagName !== "ION-PICKER-COLUMN" && relatedTarget !== this.inputEl) this.exitInputMode();
		};
		/**
		...
		*/
		this.onFocusIn = (ev) => {
			...
			*/
			if (target.tagName !== "ION-PICKER-COLUMN") return;
			/**
			...
			*/
			if (!this.actionOnClick) {
				...
				if (columnEl.numericInput) this.enterInputMode(columnEl, false);
 ...
 /**
				...
				*/
				if (inputModeColumn && inputModeColumn === ev.target) this.actionOnClick = () => {
					this.enterInputMode();
				};
					...
					const columnEl = el.querySelectorAll("ion-picker-column.picker-column-numeric-input").length === 1 ? ev.target : void 0;
					this.actionOnClick = () => {
		...
		*/
		this.enterInputMode = (columnEl, focusInput = true) => {
			...
			if (!inputEl) return;
			if (!el.querySelector("ion-picker-column.picker-column-numeric-input")) return;
			/**
		...
		this.selectSingleColumn = () => {
			...
			*/
			if (singleColumnSearchTimeout) clearTimeout(singleColumnSearchTimeout);
			...
			*/
			if (inputEl.value.length >= 3) {
				...
				inputEl.value = inputEl.value.substring(startIndex);
				this.selectSingleColumn();
				return;
			}
			/**
			...
			*/
			const findItemFromCompleteValue = options.find(({ textContent }) => {
				return textContent.replace(/^0+(?=[1-9])|0+(?=0$)/, "") === inputEl.value;
			});
			...
			*/
			if (inputEl.value.length === 2) {
				inputEl.value = inputEl.value.substring(inputEl.value.length - 1);
				this.selectSingleColumn();
			}
		};
		/**
		...
		*/
		this.searchColumn = (colEl, value, zeroBehavior = "start") => {
			...
			if (option) colEl.setValue(option.value);
			return !!option;
		};
		/**
		...
		*/
		this.onInputChange = () => {
			...
			if (inputModeColumn) this.selectSingleColumn();
			else this.selectMultiColumn();
		};
		/**
