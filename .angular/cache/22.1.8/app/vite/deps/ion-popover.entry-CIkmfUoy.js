import { c as createEvent, d as getElement, h as printIonWarning, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import { l as getElementRoot, t as addEventListener, u as hasLazyBuild, v as raf } from "./helpers-BJFnZngp-COG28xNj.js";
...
import { S as restoreRootFocusTrapAccessibility, b as prepareOverlay, d as focusFirstDescendant, l as dismiss, n as FOCUS_TRAP_DISABLE_CLASS, o as cleanupRootFocusTrapAccessibility, t as BACKDROP, u as eventMethod, w as setOverlayId, x as present } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import { t as createLockController } from "./lock-controller-B-hirT0v-wdq_Ov9y.js";
//#region node_modules/@ionic/core/dist/esm/ion-popover.entry.js
/*!
...
var getSafeAreaInsets = (doc) => {
	if (cachedInsets !== null) return cachedInsets;
...
var getPopoverDimensions = (size, contentEl, triggerEl) => {
	...
	if (size === "cover" && triggerEl) contentWidth = triggerEl.getBoundingClientRect().width;
	return {
		contentWidth,
		contentHeight
	};
...
var configureDismissInteraction = (triggerEl, triggerAction, popoverEl, parentPopoverEl) => {
	...
	const parentContentEl = getElementRoot(parentPopoverEl).querySelector(".popover-content");
	switch (triggerAction) {
		case "hover":
			dismissCallbacks = [{
				...
				*/
				...
				callback: (ev) => {
					if (document.elementFromPoint(ev.clientX, ev.clientY) === triggerEl) return;
		...
		default:
			dismissCallbacks = [{
				eventName: "click",
				callback: (ev) => {
					if (ev.target.closest("[data-ion-popover-trigger]") === triggerEl) {
						/**
				...
				},
				{
					...
					callback: (ev) => {
						...
						*/
						...
						if (!target) return;
						if (target.closest("ion-popover") !== popoverEl) popoverEl.dismiss(void 0, void 0, false);
					}
				},
				{
					/**
...
var getNextItem = (items, currentItem) => {
	return items[getIndexOfItem(items, currentItem) + 1];
};
/**
* Given an array of elements and a currently focused ion-item
...
var focusItem = (item) => {
	const button = getElementRoot(item).querySelector("button");
	if (button) raf(() => button.focus());
};
/**
...
var configureKeyboardInteraction = (popoverEl) => {
	const callback = async (ev) => {
		...
		*/
		if (targetTagName !== "ION-POPOVER" && targetTagName !== "ION-ITEM") return;
		/**
		...
		*/
		...
		switch (ev.key) {
			...
			*/
			case "ArrowLeft":
				if (await popoverEl.getParentPopover()) popoverEl.dismiss(void 0, void 0, false);
				break;
			/**
			...
			*/
			case "ArrowDown":
				...
				if (nextItem !== void 0) focusItem(nextItem);
				break;
			/**
			...
			*/
			case "ArrowUp":
				...
				if (prevItem !== void 0) focusItem(prevItem);
				break;
			/**
			...
			*/
			case "Home":
				...
				if (firstItem !== void 0) focusItem(firstItem);
				break;
			/**
			...
			*/
			case "End":
				...
				if (lastItem !== void 0) focusItem(lastItem);
				break;
			/**
...
var getPopoverPosition = (isRTL, contentWidth, contentHeight, arrowWidth, arrowHeight, reference, side, align, defaultPosition, triggerEl, event) => {
	...
	*/
	switch (reference) {
		case "event":
			if (!event) return defaultPosition;
			const mouseEv = event;
		...
		default:
			...
			*/
			...
			if (!actualTriggerEl) return defaultPosition;
...
var calculateArrowPosition = (side, arrowWidth, arrowHeight, top, left, contentWidth, contentHeight, isRTL) => {
	...
	*/
	...
	switch (side) {
		...
		case "left": return leftPosition;
		case "right": return rightPosition;
		case "start": return isRTL ? rightPosition : leftPosition;
		case "end": return isRTL ? leftPosition : rightPosition;
		default: return {
			arrowTop: 0,
			arrowLeft: 0
		};
...
var calculatePopoverSide = (side, triggerBoundingBox, contentWidth, contentHeight, arrowWidth, arrowHeight, isRTL) => {
	...
	switch (side) {
		...
		case "right": return sideRight;
		case "bottom": return {
			top: triggerBoundingBox.top + triggerBoundingBox.height + arrowHeight,
			left: triggerBoundingBox.left
		};
		case "left": return sideLeft;
		case "start": return isRTL ? sideRight : sideLeft;
		case "end": return isRTL ? sideLeft : sideRight;
	}
};
/**
* Calculates the required top/left
...
var calculatePopoverAlign = (align, side, triggerBoundingBox, contentWidth, contentHeight) => {
	switch (align) {
		case "center": return calculatePopoverCenterAlign(side, triggerBoundingBox, contentWidth, contentHeight);
		case "end": return calculatePopoverEndAlign(side, triggerBoundingBox, contentWidth, contentHeight);
		default: return {
			top: 0,
			left: 0
		};
...
var shouldShowArrow = (side, didAdjustBounds = false, ev, trigger) => {
	...
	* at the trigger.
	*/
	if (side !== "top" && side !== "bottom" && didAdjustBounds) return false;
	return true;
};
...
var iosEnterAnimation = (baseEl, opts) => {
	...
	const { contentWidth, contentHeight } = getPopoverDimensions(size, contentEl, trigger || ev?.detail?.ionShadowTarget || ev?.target);
	const { arrowWidth, arrowHeight } = getArrowDimensions(arrowEl);
	const results = getPopoverPosition(isRTL, contentWidth, contentHeight, arrowWidth, arrowHeight, reference, side, align, {
	...
	backdropAnimation.addElement(root.querySelector("ion-backdrop")).fromTo("opacity", .01, "var(--backdrop-opacity)").beforeStyles({ "pointer-events": "none" }).afterClearStyles(["pointer-events"]);
	contentAnimation.addElement(root.querySelector(".popover-arrow")).addElement(root.querySelector(".popover-content")).fromTo("opacity", .01, 1);
	return baseAnimation.easing("ease").duration(100).beforeAddWrite(() => {
		if (size === "cover") baseEl.style.setProperty("--width", `${contentWidth}px`);
		if (addPopoverBottomClass) baseEl.classList.add("popover-bottom");
		if (bottom !== void 0) {
			let bottomValue = `${bottom}px`;
			if (checkSafeAreaBottom) bottomValue = `${bottom}px + var(--ion-safe-area-bottom, 0px)`;
			contentEl.style.setProperty("bottom", `calc(${bottomValue})`);
		}
		const safeAreaLeft = " + var(--ion-safe-area-left, 0px)";
		const safeAreaRight = " - var(--ion-safe-area-right, 0px)";
		let leftValue = `${left}px`;
		if (checkSafeAreaLeft) leftValue = `${left}px${safeAreaLeft}`;
		if (checkSafeAreaRight) leftValue = `${left}px${safeAreaRight}`;
		let topValue = `${top}px`;
		if (checkSafeAreaTop) topValue = `${top}px + var(--ion-safe-area-top, 0px)`;
		contentEl.style.setProperty("top", `calc(${topValue} + var(--offset-y, 0))`);
		contentEl.style.setProperty("left", `calc(${leftValue} + var(--offset-x, 0))`);
		contentEl.style.setProperty("transform-origin", `${originY} ${originX}`);
		if (arrowEl !== null) {
			const didAdjustBounds = results.top !== top || results.left !== left;
			if (shouldShowArrow(side, didAdjustBounds, ev, trigger)) {
				arrowEl.style.setProperty("top", `calc(${arrowTop}px + var(--offset-y, 0))`);
				arrowEl.style.setProperty("left", `calc(${arrowLeft}px + var(--offset-x, 0))`);
			} else arrowEl.style.setProperty("display", "none");
		}
...
var iosLeaveAnimation = (baseEl) => {
	...
	contentAnimation.addElement(root.querySelector(".popover-arrow")).addElement(root.querySelector(".popover-content")).fromTo("opacity", .99, 0);
...
var mdEnterAnimation = (baseEl, opts) => {
	...
	const contentEl = root.querySelector(".popover-content");
	const { contentWidth, contentHeight } = getPopoverDimensions(size, contentEl, trigger || ev?.detail?.ionShadowTarget || ev?.target);
	const results = getPopoverPosition(isRTL, contentWidth, contentHeight, 0, 0, reference, side, align, {
		top: bodyHeight / 2 - contentHeight / 2,
		left: bodyWidth / 2 - contentWidth / 2,
		originX: isRTL ? "right" : "left",
		originY: "top"
	...
	let leftValue = `${left}px`;
	if (checkSafeAreaLeft) leftValue = `${left}px${safeAreaLeftCalc}`;
	if (checkSafeAreaRight) leftValue = `${left}px${safeAreaRightCalc}`;
	let topValue = `${top}px`;
	if (checkSafeAreaTop) topValue = `${top}px + var(--ion-safe-area-top, 0px)`;
	const baseAnimation = createAnimation();
	const backdropAnimation = createAnimation();
	...
	backdropAnimation.addElement(root.querySelector("ion-backdrop")).fromTo("opacity", .01, "var(--backdrop-opacity)").beforeStyles({ "pointer-events": "none" }).afterClearStyles(["pointer-events"]);
	wrapperAnimation.addElement(root.querySelector(".popover-wrapper")).duration(150).fromTo("opacity", .01, 1);
	...
	viewportAnimation.addElement(root.querySelector(".popover-viewport")).fromTo("opacity", .01, 1);
...
var mdLeaveAnimation = (baseEl) => {
	...
	const wrapperAnimation = createAnimation();
	backdropAnimation.addElement(root.querySelector("ion-backdrop")).fromTo("opacity", "var(--backdrop-opacity)", 0);
	wrapperAnimation.addElement(root.querySelector(".popover-wrapper")).fromTo("opacity", .99, 0);
	return baseAnimation.easing("ease").afterAddWrite(() => {
		baseEl.style.removeProperty("--width");
		baseEl.classList.remove("popover-bottom");
		contentEl.style.removeProperty("top");
...
var Popover = class {
	constructor(hostRef) {
		...
		*/
		...
		this.configureTriggerInteraction = () => {
			...
			if (destroyTriggerInteraction) destroyTriggerInteraction();
		...
		this.configureKeyboardInteraction = () => {
			...
			if (destroyKeyboardInteraction) destroyKeyboardInteraction();
		...
		this.configureDismissInteraction = () => {
			...
			if (destroyDismissInteraction) destroyDismissInteraction();
	...
	componentWillLoad() {
		...
		if (this.alignment === void 0) this.alignment = getIonMode(this) === "ios" ? "center" : "start";
	}
	componentDidLoad() {
		...
		*/
		if (isOpen === true) raf(() => this.present());
		if (parentPopover) addEventListener(parentPopover, "ionPopoverWillDismiss", () => {
	...
	*/
	async present(event) {
		...
		*/
		...
		this.recalculateContentOnHeaderReady();
		if (!this.keyboardEvents) this.configureKeyboardInteraction();
		this.configureDismissInteraction();
		/**
		...
		*/
		if (this.focusDescendantOnPresent) focusFirstDescendant(el);
		unlock();
	}
	/**
	...
	*/
	recalculateContentOnHeaderReady() {
		...
		if (!header || contentElements.length === 0) return;
		if (this.headerResizeObserver) {
			this.headerResizeObserver.disconnect();
			this.headerResizeObserver = void 0;
		}
		this.headerResizeObserver = new ResizeObserver(async () => {
			if (header.offsetHeight > 0) {
				this.headerResizeObserver?.disconnect();
				this.headerResizeObserver = void 0;
				for (const contentEl of contentElements) await contentEl.recalculateDimensions();
			}
		});
	...
	*/
	async dismiss(data, role, dismissParentPopover = true) {
		...
		if (dismissParentPopover && this.parentPopover) this.parentPopover.dismiss(data, role, dismissParentPopover);
	...
	*/
	...
	render() {
		...
		return h(Host, {
			...
			part: "content"
		}, h("slot", { key: "b29747419620a01a6c34e9e846cd529b0b210145" }))));
	}
	get el() {
