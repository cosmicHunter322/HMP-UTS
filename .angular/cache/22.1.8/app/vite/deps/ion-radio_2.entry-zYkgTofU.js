...
import { b as renderHiddenInput, t as addEventListener, y as removeEventListener } from "./helpers-BJFnZngp-COG28xNj.js";
...
import { n as isOptionSelected } from "./compare-with-utils-sObYyvOy-PfxWXB4Z.js";
//#region node_modules/@ionic/core/dist/esm/ion-radio_2.entry.js
/*!
...
var Radio = class {
	constructor(hostRef) {
		...
		*/
		...
		this.onClick = () => {
			...
			*/
			if (checked && radioGroup?.allowEmptySelection) this.checked = false;
	...
	connectedCallback() {
		if (this.value === void 0) this.value = this.inputId;
	...
	render() {
		...
		return h(Host, {
			key: "ad7ac0bdc4d3216ecc19402e5b8d6f9e4d09334b",
			...
			key: "59c09125633120e2000447b8a3bc24c54490aa85",
			class: "radio-wrapper"
		}, h("div", {
			key: "1a98f3e0027deeac52647ba37aa9a99be0231966",
			class: {
				"label-text-wrapper": true,
				"label-text-wrapper-hidden": !hasLabel
			},
			part: "label"
		}, h("slot", { key: "6a48e76ac8793d2aacebd3a8f193e3537272e5c2" })), h("div", {
			key: "68e4350bac8dad29c6b7a180560de2bb4dfc8c41",
...
var RadioGroup = class {
	...
	async connectedCallback() {
		...
		if (header) {
			...
			if (label) this.labelId = label.id = this.name + "-lbl";
		}
	...
	*/
	...
	onKeydown(ev) {
		...
		if (ev.target && !this.el.contains(ev.target)) return;
		const radios = this.getRadios().filter((radio) => !radio.disabled);
		if (ev.target && radios.includes(ev.target)) {
			const index = radios.findIndex((radio) => radio === ev.target);
			const current = radios[index];
			let next;
			if (["ArrowDown", "ArrowRight"].includes(ev.key)) next = index === radios.length - 1 ? radios[0] : radios[index + 1];
			if (["ArrowUp", "ArrowLeft"].includes(ev.key)) next = index === 0 ? radios[radios.length - 1] : radios[index - 1];
 ...
 /**
			...
			if (ev.key === "Enter" && inSelectInterface && !ev.repeat) {
				...
				if (previousValue !== this.value) this.emitValueChange(ev);
				ev.preventDefault();
			}
		}
	}
	/** @internal */
	async setFocus() {
		this.getRadios().find((r) => r.tabIndex !== -1)?.setFocus();
	}
	/**
	...
	*/
	renderHintText() {
		...
		return h("div", { class: "radio-group-top" }, h("div", {
			id: helperTextId,
			class: "helper-text",
			"aria-live": "polite"
	...
	render() {
		...
		return h(Host, {
			key: "8d926a17e800adf0184274215c9689c96a8d6dfd",
			role: "radiogroup",
			...
			onClick: this.onClick,
			class: mode
		}, this.renderHintText(), h("slot", { key: "2ff891123d5971a581b70d0886d23a074358c2dc" }));
	}
	get el() {
		return getElement(this);
	}
