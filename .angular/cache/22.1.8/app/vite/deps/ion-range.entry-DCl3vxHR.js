...
import { b as renderHiddenInput, f as inheritAriaAttributes, h as isSafeNumber, o as debounceEvent, r as clamp } from "./helpers-BJFnZngp-COG28xNj.js";
...
import { d as resetContentScrollY, i as findClosestIonContent, r as disableContentScrollY } from "./index-obGyiHiO-C73DG7Nj.js";
//#region node_modules/@ionic/core/dist/esm/ion-range.entry.js
/*!
...
var Range = class {
	constructor(hostRef) {
		...
		*/
		this.compareValues = (newVal, oldVal) => {
			if (typeof newVal === "object" && typeof oldVal === "object") return newVal.lower !== oldVal.lower || newVal.upper !== oldVal.upper;
		...
		this.ensureValueInBounds = (value) => {
			if (this.dualKnobs) return {
				lower: this.clampBounds(value.lower),
				upper: this.clampBounds(value.upper)
			};
			else return this.clampBounds(value);
		};
		/**
		...
		*/
		...
		this.handleKeyboard = (knob, isIncrease) => {
			...
			if (!isIncrease) step *= -1;
			if (knob === "A") this.ratioA = clamp(0, this.ratioA + step, 1);
			else this.ratioB = clamp(0, this.ratioB + step, 1);
		...
		this.onKnobFocus = (knob) => {
			if (!this.focusFromPointer) this.focusedKnob = knob;
			else {
				this.focusFromPointer = false;
				this.focusedKnob = void 0;
			}
		...
		this.onKnobBlur = () => {
			setTimeout(() => {
				...
				if (!(activeElement && activeElement.classList.contains("range-knob-handle"))) {
					if (this.hasFocus) {
						this.hasFocus = false;
						this.focusedKnob = void 0;
						this.ionBlur.emit();
	...
	dualKnobsChanged() {
		if (!this.noUpdate) this.updateRatio();
	}
	minChanged(newValue) {
		if (!isSafeNumber(newValue)) this.min = 0;
		if (!this.noUpdate) this.updateRatio();
	}
	maxChanged(newValue) {
		if (!isSafeNumber(newValue)) this.max = 100;
		if (!this.noUpdate) this.updateRatio();
	}
	stepChanged(newValue) {
		if (!isSafeNumber(newValue)) this.step = 1;
	}
	...
	valueChanged(newValue, oldValue) {
		if (this.compareValues(newValue, oldValue)) this.ionInput.emit({ value: this.value });
		if (!this.noUpdate) this.updateRatio();
	}
	componentWillLoad() {
		/**
		...
		*/
		if (this.el.hasAttribute("id")) this.rangeId = this.el.getAttribute("id");
	...
	getValue() {
		...
		if (this.dualKnobs) {
			...
			if (typeof value === "object") return value.upper;
			return value;
		}
	}
	/**
	...
	*/
	onMove(detail) {
		...
		*/
		if (contentEl && this.pressedKnob === void 0) this.initialContentScrollY = disableContentScrollY(contentEl);
		/**
		...
		*/
		if (pressedKnob === void 0) this.setPressedKnob(currentX);
	...
	*/
	onEnd(detail) {
		...
		* Plus, it determines when to apply certain styles.
		*/
		if (this.pressedKnob === void 0) this.setPressedKnob(currentX);
		/**
		...
		*/
		if (contentEl && this.pressedKnob !== void 0) resetContentScrollY(contentEl, initialContentScrollY);
		this.update(currentX);
		/**
	...
	update(currentX) {
		...
		if (isRTL(this.el)) ratio = 1 - ratio;
		if (this.snaps) ratio = valueToRatio(ratioToValue(ratio, this.min, this.max, this.step), this.min, this.max);
		if (this.pressedKnob === "A") this.ratioA = ratio;
		else this.ratioB = ratio;
		this.updateValue();
	}
	...
	get ratioLower() {
		...
		const { activeBarStart } = this;
		if (activeBarStart == null) return 0;
		return valueToRatio(activeBarStart, this.min, this.max);
	}
	get ratioUpper() {
		if (this.dualKnobs) return Math.max(this.ratioA, this.ratioB);
		return this.ratioA;
	}
	updateRatio() {
		...
		*/
		...
		} else this.ratioA = valueToRatio(value, min, max);
	}
	updateValue() {
		this.noUpdate = true;
 ...
 /**
		...
		*/
		...
		const ticks = [];
		if (this.snaps && this.ticks) for (let value = min; value <= max; value += step) {
		...
		})), h("div", { class: "range-bar-container" }, h("div", {
			class: "range-bar",
			role: "presentation",
			part: "bar"
		}), h("div", {
	...
	render() {
		...
		*/
		...
		return h(Host, {
			key: "72f253ee1514b4c5874a6720c2f954fae7a7c4d4",
			...
			key: "9b96b83a6118ee2a79d8adbb0a58197470308179",
			class: "range-wrapper",
			id: "range-label"
		}, h("div", {
			key: "c7b31a7c87c20857a26624bd94501df6420fceec",
			class: {
				"label-text-wrapper": true,
				"label-text-wrapper-hidden": !hasLabel
			},
			part: "label"
		}, label !== void 0 ? h("div", { class: "label-text" }, label) : h("slot", { name: "label" })), h("div", {
			key: "4ca15b9370097e140550489ff24979f3a959e65b",
			class: "native-wrapper"
		}, h("slot", {
			key: "f110896c68741bad92239f98b6028cbe0ad5122f",
			name: "start"
		}), this.renderRangeSlider(), h("slot", {
			key: "5c4031c4403fa643ee2e11637b2420ddd8cbc4ce",
			name: "end"
		}))));
	}
	get el() {
	...
	static get watchers() {
		return {
			...
			"dualKnobs": [{ "dualKnobsChanged": 0 }],
			"min": [{ "minChanged": 0 }],
			"max": [{ "maxChanged": 0 }],
			"step": [{ "stepChanged": 0 }],
			"activeBarStart": [{ "activeBarStartChanged": 0 }],
...
var getKnobPosition = (knob, ratioA, ratioB, dualKnobs) => {
	if (!dualKnobs) return "lower";
	if (ratioA === ratioB) return knob === "A" ? "lower" : "upper";
	if (knob === "A") return ratioA < ratioB ? "lower" : "upper";
	return ratioB < ratioA ? "lower" : "upper";
};
var ratioToValue = (ratio, min, max, step) => {
	let value = (max - min) * ratio;
	if (step > 0) value = Math.round(value / step) * step + min;
	return roundToMaxDecimalPlaces(clamp(min, value, max), min, max, step);
};
