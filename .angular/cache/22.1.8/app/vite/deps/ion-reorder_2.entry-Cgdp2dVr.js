...
import { c as getScrollElement, i as findClosestIonContent } from "./index-obGyiHiO-C73DG7Nj.js";
import "./capacitor-C041I3qZ-AKn9tKTZ.js";
import { C as reorderTwoSharp, S as reorderThreeOutline } from "./index-BvabGprD-DxcnQO2v.js";
import { a as hapticSelectionEnd, i as hapticSelectionChanged, o as hapticSelectionStart } from "./haptic-D3njgGLB-CR0w36Fl.js";
//#region node_modules/@ionic/core/dist/esm/ion-reorder_2.entry.js
/*!
...
var Reorder = class {
	...
	onClick(ev) {
		...
		ev.preventDefault();
		if (!reorderGroup || !reorderGroup.disabled) ev.stopImmediatePropagation();
	}
	render() {
		const mode = getIonMode(this);
		return h(Host, {
			key: "f278c473b4695a6a93565100e4fcaa7f9e3e9ff5",
			class: mode
		}, h("slot", { key: "cc26b5ed297f68a0bea82e27e34a7fe945186f4e" }, h("ion-icon", {
			key: "696f5e4c2b09e64bd08abf0d0818b1554cd29737",
			icon: mode === "ios" ? reorderThreeOutline : reorderTwoSharp,
			lazy: false,
			class: "reorder-icon",
			part: "icon",
			"aria-hidden": "true"
...
var ReorderGroup = class {
	...
	async connectedCallback() {
		...
		if (contentEl) this.scrollEl = await getScrollElement(contentEl);
	...
	*/
	...
	canStart(ev) {
		if (this.selectedItemEl || this.state !== 0) return false;
		const reorderEl = ev.event.target.closest("ion-reorder");
		if (!reorderEl) return false;
	...
	onEnd() {
		...
		if (toIndex === fromIndex) this.completeReorder();
		else this.ionItemReorder.emit({
			from: fromIndex,
			to: toIndex,
		...
		if (selectedItemEl && this.state === 2) {
			...
			*/
			raf(() => {
				...
				for (let i = 0; i < len; i++) children[i].style["transform"] = "";
			});
			if (Array.isArray(listOrReorder)) listOrReorder = reorderArray(listOrReorder, fromIndex, toIndex);
	...
	itemIndexForTop(deltaY) {
		const heights = this.cachedHeights;
		for (let i = 0; i < heights.length; i++) if (heights[i] > deltaY) return i;
	...
	reorderMove(fromIndex, toIndex) {
		...
		for (let i = 0; i < children.length; i++) {
			...
			let value = "";
			if (i > fromIndex && i <= toIndex) value = `translateY(${-itemHeight}px)`;
			else if (i < fromIndex && i >= toIndex) value = `translateY(${itemHeight}px)`;
	...
	autoscroll(posY) {
		...
		let amount = 0;
		if (posY < this.scrollElTop) amount = -SCROLL_JUMP;
		else if (posY > this.scrollElBottom) amount = SCROLL_JUMP;
		if (amount !== 0) this.scrollEl.scrollBy(0, amount);
		return this.scrollEl.scrollTop - this.scrollElInitial;
	}
	render() {
		return h(Host, {
			key: "779332508f44409696b6bab80eda72f9b1d7869d",
			class: {
