import { c as createEvent, d as getElement, h as printIonWarning, l as forceUpdate, m as printIonError, p as h, r as Host, v as registerInstance, x as writeTask } from "./index-Dvqtt_WK-CtfmC4By.js";
import { t as isRTL } from "./dir-Dojwmvde-JWsS7_L7.js";
import { p as inheritAttributes, t as addEventListener, v as raf, y as removeEventListener } from "./helpers-BJFnZngp-COG28xNj.js";
...
//#region node_modules/@ionic/core/dist/esm/ion-segment_2.entry.js
/*!
...
var Segment = class {
	constructor(hostRef) {
		...
		*/
		...
		this.onClick = (ev) => {
			...
			if (current.tagName === "ION-SEGMENT") return;
			this.value = current.value;
			if (current !== previous) this.emitValueChange();
			if (this.segmentViewEl) {
				this.updateSegmentView();
				if (this.scrollable && previous) this.checkButton(previous, current);
			} else if (this.scrollable || !this.swipeGesture) if (previous) this.checkButton(previous, current);
			else this.setCheckedClasses();
		};
		...
		this.getSegmentButton = (selector) => {
			...
			switch (selector) {
				case "first": return buttons[0];
				case "last": return buttons[buttons.length - 1];
				case "next": return buttons[currIndex + 1] ?? buttons[0];
				case "previous": return buttons[currIndex - 1] ?? buttons[buttons.length - 1];
				default: return null;
			}
		};
	}
	colorChanged(value, oldValue) {
		...
		*/
		if (oldValue === void 0 && value !== void 0 || oldValue !== void 0 && value === void 0) this.emitStyle();
	}
	swipeGestureChanged() {
		this.gestureChanged();
	}
	valueChanged(value, oldValue) {
		...
		if (oldValue !== void 0 && value !== void 0) {
			...
			if (previous && current) {
				if (!this.segmentViewEl) this.checkButton(previous, current);
				else if (this.triggerScrollOnValueChange !== false) this.updateSegmentView();
			}
		} else if (value !== void 0 && oldValue === void 0 && this.segmentViewEl) this.updateSegmentView();
		/**
		* `ionSelect` is emitted every time the value changes (internal or external changes).
		* Used by `ion-segment-button` to determine if the button should be checked.
		*/
		this.ionSelect.emit({ value });
		if (!this.segmentViewEl) this.scrollActiveButtonIntoView();
	...
	disabledChanged() {
		...
		if (!this.segmentViewEl) {
			const buttons = this.getButtons();
			for (const button of buttons) button.disabled = this.disabled;
		} else this.segmentViewEl.disabled = this.disabled;
	}
	gestureChanged() {
		if (this.gesture) this.gesture.enable(!this.scrollable && !this.disabled && this.swipeGesture);
	}
	connectedCallback() {
		this.emitStyle();
	...
	*/
	...
	activate(detail) {
		...
		const checked = this.getButtons().find((button) => button.value === this.value);
		if (clicked.tagName !== "ION-SEGMENT-BUTTON") return;
		if (!checked) {
			this.value = clicked.value;
			this.setCheckedClasses();
		}
		if (this.value === clicked.value) this.setActivated(true);
	}
	getIndicator(button) {
		return (button.shadowRoot || button).querySelector(".segment-button-indicator");
	}
	checkButton(previous, current) {
		...
		const transform = `translate3d(${previousClientRect.left - currentClientRect.left}px, 0, 0) scaleX(${widthDelta})`;
		writeTask(() => {
	...
	setCheckedClasses() {
		const buttons = this.getButtons();
		const next = buttons.findIndex((button) => button.value === this.value) + 1;
		for (const button of buttons) button.classList.remove("segment-button-after-checked");
		if (next < buttons.length) buttons[next].classList.add("segment-button-after-checked");
	}
	getSegmentView() {
		const firstContentId = this.getButtons().find((button) => button.contentId);
		return document.querySelector(`ion-segment-content[id="${firstContentId?.contentId}"]`)?.closest("ion-segment-view");
	}
	handleSegmentViewScroll(ev) {
		...
		if (ev.composedPath().includes(segmentViewEl) || dispatchedFrom?.contains(segmentEl)) {
			...
			const current = buttons[buttons.findIndex((button) => button.value === this.value)];
	...
	*/
	updateSegmentView(smoothScroll = true) {
		const button = this.getButtons().find((btn) => btn.value === this.value);
		if (!button?.contentId) return;
		const segmentView = this.segmentViewEl;
		if (segmentView) segmentView.setContent(button.contentId, smoothScroll);
	}
	scrollActiveButtonIntoView(smoothScroll = true) {
		...
		if (scrollable) {
			...
			if (activeButton !== void 0) {
				...
				* scroll container.
				*/
				const centeredX = activeButtonBox.x - scrollContainerBox.x - scrollContainerBox.width / 2 + activeButtonBox.width / 2;
				/**
	...
	setNextIndex(detail, isEnd = false) {
		...
		const nextEl = this.el.getRootNode().elementFromPoint(currentX, previousY);
		const decreaseIndex = rtl ? currentX > left + width : currentX < left;
		const increaseIndex = rtl ? currentX < left : currentX > left + width;
		if (activated && !isEnd) {
			if (decreaseIndex) {
				const newIndex = index - 1;
				if (newIndex >= 0) nextIndex = newIndex;
			} else if (increaseIndex) {
				if (activated && !isEnd) {
					const newIndex = index + 1;
					if (newIndex < buttons.length) nextIndex = newIndex;
				}
			}
			if (nextIndex !== void 0 && !buttons[nextIndex].disabled) current = buttons[nextIndex];
		}
		if (!activated && isEnd) current = nextEl;
		if (current != null) {
			/**
			...
			*/
			if (current.tagName === "ION-SEGMENT") return false;
			if (previous !== current) this.checkButton(previous, current);
		}
		return true;
	}
	emitStyle() {
	...
	render() {
		...
		return h(Host, {
			key: "f74bd72f10f966129528497aa47b2c683a62d59e",
			role: "tablist",
			...
			key: "79b0c6cce9d83d147b190ae5d77df7de29a6b42d",
...
var SegmentButton = class {
		...
		*/
		...
		this.updateState = () => {
			...
			if (segmentEl) {
				...
				if (segmentEl.disabled) this.disabled = true;
			}
		};
	}
	valueChanged() {
	...
	*/
	async setFocus() {
		const { nativeEl } = this;
		if (nativeEl !== void 0) nativeEl.focus();
	}
	render() {
		...
		return h(Host, {
			key: "d8acc71d87e26b32f20b2d0f9a6acc9f0dab0dee",
			class: {
				[mode]: true,
				"in-toolbar": hostContext("ion-toolbar", this.el),
				"in-toolbar-color": hostContext("ion-toolbar[color]", this.el),
			...
			key: "ce65da07b1a012bce353b3c6c24eecfbd4071d54",
			...
			key: "6c4b4dcd20e407e5dc31674bea23633d8a09187a",
			class: "button-inner"
		}, h("slot", { key: "817145ae77a3c5ada88ab95723d881f1bde96b2a" })), mode === "md" && h("ion-ripple-effect", { key: "054d7e5e2a13013914fac6bbde8efdb28d9b459e" })), h("div", {
			key: "c48074881a302510ad4752eeb9383222cd3a54cb",
			part: "indicator",
			class: "segment-button-indicator segment-button-indicator-animated"
		}, h("div", {
			key: "7672a6b10ee18ad026b798fbd8207dc3cd5683f9",
