import { a as config, c as createEvent, d as getElement, h as printIonWarning, l as forceUpdate, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import { t as isRTL } from "./dir-Dojwmvde-JWsS7_L7.js";
import { b as renderHiddenInput, c as focusVisibleElement, p as inheritAttributes } from "./helpers-BJFnZngp-COG28xNj.js";
import "./gesture-controller-B_gJaBk0-B8pQAih_.js";
import { t as getIonMode } from "./ionic-global-BSaFA7np-C0AYHzhu.js";
import { a as sanitizeDOMTree, n as blockedTags, r as reflectPropertiesToAttributes } from "./index-DmnOg_xm-CoMu4g4x.js";
import { n as getClassMap, r as hostContext, t as createColorClasses } from "./theme-byZM6qHV-CVcL0HyM.js";
import "./framework-delegate-CUDbUfWS-SLUux__8.js";
import { C as safeCall, _ as modalController, a as alertController, i as actionSheetController, y as popoverController } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import "./index-BvabGprD-DxcnQO2v.js";
import { t as renderOptionLabel } from "./select-option-render-C_PCwA67-1dMTsVmU.js";
import { n as getOverlayLabelPlacement, t as getOverlayLabelJustify } from "./overlay-control-label-BSQPZ79H-C3sbMJlx.js";
import { t as checkInvalidState } from "./validity-DJztqcrH-if6nl8R-.js";
import { a as getSlottedClickContent, i as createStartContainerController, n as createNotchController, r as createSlotMutationController, t as createClickController } from "./slot-mutation-controller-D7n8Peo2-BISKMHTP.js";
import { t as watchForOptions } from "./watch-options-Dtdm8lKC-Dz-LUoDw.js";
import { n as isOptionSelected, t as compareOptions } from "./compare-with-utils-sObYyvOy-PfxWXB4Z.js";
//#region node_modules/@ionic/core/dist/esm/ion-select_3.entry.js
/*!
...
var Select = class {
	constructor(hostRef) {
		...
		this.customHTMLEnabled = config.get("innerHTMLTemplatesEnabled", false);
		...
		this.onClick = (ev) => {
			...
			*/
			if (slotted !== null) {
				...
				if (interactive !== null && slotted.contains(interactive)) return;
			}
			this.setFocus();
			this.open(ev);
		};
		...
		*/
		this.onLabelClick = (ev) => {
			if (ev.target === this.focusEl && this.isExpanded) ev.stopPropagation();
		};
	}
	styleChanged() {
		this.emitStyle();
	...
	setValue(value) {
		if (this.isValueEqual(this.value, value)) return;
		this.value = value;
	...
	isValueEqual(currentValue, newValue) {
		if (this.multiple) {
			...
			if (currentArr.length !== newArr.length) return false;
	...
	async connectedCallback() {
		const { el } = this;
		this.slotMutationController = createSlotMutationController(el, [
			"label",
			"start",
			"end"
		], () => {
	...
	*/
	async open(event) {
		...
		if (this.interface === "modal") overlay.addEventListener("ionModalWillPresent", scrollSelectedIntoView, { once: true });
		else if (this.interface === "popover") overlay.addEventListener("ionPopoverWillPresent", scrollSelectedIntoView, { once: true });
		else {
			/**
			...
			*/
			...
			if (this.interface === "alert") overlay.addEventListener("ionAlertWillPresent", scrollAfterRender, { once: true });
			else if (this.interface === "action-sheet") overlay.addEventListener("ionActionSheetWillPresent", scrollAfterRender, { once: true });
		}
	...
	createOverlay(ev) {
		...
		if (selectInterface === "action-sheet") return this.openActionSheet();
		if (selectInterface === "popover") return this.openPopover(ev);
		if (selectInterface === "modal") return this.openModal();
	...
	updateOverlayOptions() {
		...
		switch (this.interface) {
			...
			case "popover":
				...
				if (popover) popover.options = this.createOverlaySelectOptions(childOpts, value);
				break;
			case "modal":
				const modal = overlay.querySelector("ion-select-modal");
				if (modal) modal.options = this.createOverlaySelectOptions(childOpts, value);
				break;
			case "alert":
	...
	createActionSheetButtons(data, selectValue) {
		const actionSheetButtons = data.map((option) => {
			...
			const optClass = `${OPTION_CLASS} ${Array.from(option.classList).filter((cls) => cls !== "hydrated").join(" ")}`;
	...
	async openPopover(ev) {
		...
		*/
		if (labelPlacement === "floating" || labelPlacement === "stacked" || mode === "md" && fill !== void 0) size = "cover";
		else event = {
			...ev,
	...
	*/
	get labelText() {
		...
		if (labelSlot !== null) return labelSlot.textContent;
	}
	/**
	...
	*/
	getText(useHTML = false) {
		...
		if (selectedText != null && selectedText !== "") return selectedText;
	...
	emitStyle() {
		const { disabled } = this;
		const style = { "interactive-disabled": disabled };
	...
	*/
	renderOutlineContainer() {
		...
		}, this.label)), h("div", { class: "select-outline-end" }));
	}
	/**
	...
	*/
	wrapSelectTextNodes(html) {
		...
		temp.innerHTML = html;
		if (!Array.from(temp.childNodes).some((n) => n.nodeType === Node.ELEMENT_NODE)) return html;
		Array.from(temp.childNodes).forEach((node) => {
			if (node.nodeType === Node.TEXT_NODE && node.textContent?.trim()) {
				...
				*/
				const commaMatch = node.textContent.match(/^(,\s*)(.*)/);
				if (commaMatch) {
	...
	*/
	renderSelectText() {
		...
		if (this.customHTMLEnabled) return h("div", {
			"aria-hidden": "true",
			class: selectTextClasses,
			part: textPart,
			innerHTML: this.wrapSelectTextNodes(selectText)
		});
		return h("div", {
			"aria-hidden": "true",
			class: selectTextClasses,
	...
	*/
	renderSelectIcon() {
		...
		let icon;
		if (isExpanded && expandedIcon !== void 0) icon = expandedIcon;
		else icon = toggleIcon ?? (mode === "ios" ? "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='ionicon'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='48' d='m136 208 120-104 120 104M136 304l120 104 120-104' class='ionicon-fill-none'/></svg>" : "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 512 512' class='ionicon'><path d='m64 144 192 224 192-224z'/></svg>");
		return h("ion-icon", {
			class: "select-icon",
			part: "icon",
			"aria-hidden": "true",
	...
	get ariaLabel() {
		...
		*/
		...
		if (renderedLabel === "" && placeholder !== void 0) renderedLabel = placeholder;
		/**
		...
		*/
		if (definedLabel !== void 0) renderedLabel = renderedLabel === "" ? definedLabel : `${definedLabel}, ${renderedLabel}`;
	...
	getHintTextId() {
		...
		if (isInvalid && errorText) return errorTextId;
	...
	*/
	renderBottomContent() {
		...
		if (!(!!helperText || !!errorText)) return;
		return h("div", {
			class: "select-bottom",
			part: "bottom"
		}, this.renderHintText());
	...
	render() {
		...
		return h(Host, {
			key: "31585557ac49c439939b5a084867f5039dc2f1ce",
			...
			key: "368f5f0c77550be6a743752dd1acbfe9915f0a3d",
			class: "select-wrapper",
			id: "select-label",
			onClick: this.onLabelClick,
			part: "wrapper"
		}, hasOutlineFill && this.renderOutlineContainer(), h("div", {
			key: "2e9039eaa15f398ebe54808ff659db89d3621681",
			class: "select-start",
			part: "start",
			ref: (el) => this.startContainerEl = el
		}, h("slot", {
			key: "b83e362f89bd8cbc564ad7122778e3a0f08be66d",
			name: "start"
		})), h("div", {
			key: "f55ac9fc3bbd9a4a3819a101a3780fc02ae8a8e0",
			class: "select-control",
			part: "control"
		}, this.renderLabel(), h("div", {
			key: "e32984e57cda5cf341af8e354a2aef5a114827c0",
			class: "native-wrapper",
			ref: (el) => this.nativeWrapperEl = el,
			part: "container"
		}, this.renderSelectText(), this.renderListbox(), !hasFloatingOrStackedLabel && this.renderSelectIcon())), h("div", {
			key: "fc4cb7c392c46129afe709d9557ad9a10d72b564",
			class: "select-end",
			part: "end"
		}, hasFloatingOrStackedLabel && this.renderSelectIcon(), h("slot", {
			key: "6d65c92cdafe985a769fb3b41a58625077803eef",
			name: "end"
		})), shouldRenderHighlight && h("div", {
			key: "9a912c64d7cd77f074a069d126cd0bada1b71eed",
...
var generateText = (opts, value, compareWith, useHTML = false) => {
	...
	if (Array.isArray(value)) return value.map((v) => textForValue(opts, v, compareWith, useHTML)).filter((opt) => opt !== null).join(", ");
	else return textForValue(opts, value, compareWith, useHTML) || "";
};
/**
...
var textForValue = (opts, value, compareWith, useHTML = false) => {
	...
	const customHTMLEnabled = config.get("innerHTMLTemplatesEnabled", false);
	if (!selectOpt) return null;
	if (customHTMLEnabled && useHTML) return getOptionContent(selectOpt, void 0, true);
	/**
...
var trimTextNodes = (node) => {
	node.childNodes.forEach((child) => {
		...
		else if (child.nodeType === Node.ELEMENT_NODE) trimTextNodes(child);
	});
};
/**
...
var getOptionContent = (option, slotName, useHTML = false) => {
	let nodes;
	if (slotName) nodes = Array.from(option.children).filter((el) => el.getAttribute("slot") === slotName);
	else nodes = (getOptionDefaultSlot(option) || []).filter((node) => {
		/**
	...
	*/
	if (!slotName && nodes.every((n) => n.nodeType === Node.TEXT_NODE)) return getDefaultSlotPlainText(option) || null;
	/**
	...
	*/
	nodes.forEach((n) => {
		if (n.nodeType === Node.ELEMENT_NODE) reflectPropertiesToAttributes(n);
	});
	const container = document.createElement("div");
	nodes.forEach((n) => {
		const clone = n.cloneNode(true);
		if (clone.nodeType === Node.TEXT_NODE) clone.textContent = clone.textContent?.trim() || "";
		else trimTextNodes(clone);
	...
	*/
	...
	if (useHTML) return container.innerHTML.trim() || null;
	return container;
};
/**
...
var getOptionDefaultSlot = (option) => {
	...
	if (defaultSlotNodes.length === 0) return null;
...
var getRenderedTextContent = (node) => {
	...
	if (blockedTags.includes(node.tagName.toLowerCase())) return "";
...
var getDefaultSlotPlainText = (option) => {
	return (getOptionDefaultSlot(option) ?? []).map((node) => getRenderedTextContent(node)).join("").replace(/[ \t\n\r\f]+/g, " ").replace(/^[ \t\n\r\f]+|[ \t\n\r\f]+$/g, "");
};
/**
...
*/
var INTERACTIVE_SLOTTED_CONTENT = [
	"a[href]",
	"button",
	"input[type=\"checkbox\"]",
	"input[type=\"radio\"]",
...
var SelectOption = class {
	...
	render() {
		return h(Host, {
			key: "01275b5a613c4b065b24e0d939f611f6ed1fc9ee",
			role: "option",
			id: this.inputId,
...
var SelectPopover = class {
	...
	*/
	callOptionHandler(ev) {
		...
		if (option?.handler) safeCall(option.handler, values);
	}
	/**
	...
	*/
	...
	setChecked(ev) {
		const { multiple } = this;
		const option = this.findOptionFromEvent(ev);
		if (multiple && option) option.checked = ev.detail.checked;
	}
	getValues(ev) {
		const { multiple, options } = this;
		if (multiple) return options.filter((o) => o.checked).map((o) => o.value);
		const option = this.findOptionFromEvent(ev);
		...
		return options.map((option, index) => {
			...
			*/
			...
			return h("ion-item", {
				...
				class: { "select-option-has-rich-content": hasRichContent },
				value: option.value,
				disabled: option.disabled,
		...
		return h("ion-radio-group", {
			...
			*/
			...
			return h("ion-item", {
				...
				onKeyDown: (ev) => {
					if (ev.key === "Enter" && !ev.repeat) this.pendingEnterTarget = ev.currentTarget;
				},
				onKeyUp: (ev) => {
					if (ev.key === " ") this.dismissParentPopover();
					else if (ev.key === "Enter") {
						const shouldDismiss = this.pendingEnterTarget === ev.currentTarget;
						this.pendingEnterTarget = null;
						if (shouldDismiss) this.dismissParentPopover();
					}
				}
			}, renderOptionLabel(optionLabelOptions, "select-option-label")));
		}));
	...
	render() {
		...
		return h(Host, {
			key: "4b4ac4a4b6523147e7c1f5e29dd8043867f2baea",
			class: getIonMode(this)
		}, h("ion-list", { key: "f700d3dc3ce68c20a4d981164b29262564097fb4" }, header !== void 0 && h("ion-list-header", { key: "196bcd7963f13ef8bc46231545eee4cb8d526f5e" }, header), hasSubHeaderOrMessage && h("ion-item", { key: "bb108e0aa2409997df35f52cb67e58e55e0fd630" }, h("ion-label", {
			key: "59e8d9196ff976f3835766662654c0f085e58ec3",
			class: "ion-text-wrap"
		}, subHeader !== void 0 && h("h3", { key: "532f75d7c129ffadca4e6cbb619a4427aaede65c" }, subHeader), message !== void 0 && h("p", { key: "fc42c91594b9001e983cb5e4176fc4c9bff0f708" }, message))), this.renderOptions(options)));
	}
	get el() {
		return getElement(this);
	}
