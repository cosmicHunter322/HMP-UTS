import { c as createEvent, d as getElement, m as printIonError, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import "./helpers-BJFnZngp-COG28xNj.js";
import { n as attachComponent } from "./framework-delegate-CUDbUfWS-SLUux__8.js";
//#region node_modules/@ionic/core/dist/esm/ion-tab_2.entry.js
/*!
...
var Tab = class {
	...
	async componentWillLoad() {
		if (this.active) await this.setActive();
	}
	...
	changeActive(isActive) {
		if (isActive) this.prepareLazyLoaded().catch((e) => {
			printIonError("[ion-tab] - Exception in prepareLazyLoaded:", e);
		});
	}
	prepareLazyLoaded() {
		if (this.component == null) return Promise.resolve(void 0);
		/**
		...
		*/
		if (this.loadPromise === void 0) this.loadPromise = attachComponent(this.delegate, this.el, this.component, ["ion-page"]).catch((e) => {
	...
	render() {
		...
		return h(Host, {
			key: "8ecbd7a85d3370ed8ed40561cd8134e71584d6aa",
			role: "tabpanel",
 ...
 /**
		...
		*/
		...
		if (!this.useRouter) {
			...
			if (tabs.length > 0) await this.select(tabs[0]);
		}
	...
	updateTabBar() {
		...
		if (tabBar.selectedTab === tab) return;
	...
	*/
	async select(tab) {
		...
		if (!this.shouldSwitch(selectedTab)) return false;
	...
	*/
	...
	async setRouteId(id) {
		...
		if (!this.shouldSwitch(selectedTab)) return {
			changed: false,
	...
	setActive(selectedTab) {
		if (this.transitioning) return Promise.reject("transitioning already happening");
	...
	tabSwitch() {
		...
		if (leavingTab !== selectedTab) {
			if (leavingTab) leavingTab.active = false;
	...
	notifyRouter() {
		if (this.useRouter) {
			const router = document.querySelector("ion-router");
			if (router) return router.navChanged("forward");
		}
	...
	render() {
		return h(Host, {
			key: "7b8c6455e0673e343d0c759ed85e0d9970de3bb8",
			onIonTabButtonClick: this.onTabClicked
		}, h("slot", {
			key: "7d0d840ae03312ca22bcb694ffca738d6648218e",
			name: "top"
		}), h("div", {
			key: "f336d903800af34af0d36adfc94223b940111e6a",
			class: "tabs-inner"
		}, h("slot", { key: "14656154959a24ae176eff20f6463043180d2eae" })), h("slot", {
			key: "2be4d114f058d24b0e70fb12cfb0eccd3d6e6a74",
			name: "bottom"
		}));
	}
	get el() {
...
var getTab = (tabs, tab) => {
	...
	if (!tabEl) printIonError(`[ion-tabs] - Tab with id: "${tabEl}" does not exist`);
	return tabEl;
};
Tabs.style = tabsCss();
//#endregion
