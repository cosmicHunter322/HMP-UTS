import { a as config, c as createEvent, d as getElement, h as printIonWarning, m as printIonError, p as h, r as Host, v as registerInstance } from "./index-Dvqtt_WK-CtfmC4By.js";
import { C as win, l as getElementRoot, v as raf } from "./helpers-BJFnZngp-COG28xNj.js";
...
import { n as getClassMap, t as createColorClasses } from "./theme-byZM6qHV-CVcL0HyM.js";
import "./framework-delegate-CUDbUfWS-SLUux__8.js";
import { C as safeCall, b as prepareOverlay, c as createTriggerController, h as isCancel, l as dismiss, r as GESTURE, s as createDelegateController, u as eventMethod, w as setOverlayId, x as present } from "./overlays-a_5AhqWk-BAKKNgyR.js";
import { t as createLockController } from "./lock-controller-B-hirT0v-wdq_Ov9y.js";
//#region node_modules/@ionic/core/dist/esm/ion-toast.entry.js
/*!
...
function getAnimationPosition(position, positionAnchor, mode, toast) {
	...
	*/
	let offset;
	if (mode === "md") offset = position === "top" ? 8 : -8;
	else offset = position === "top" ? 10 : -10;
	/**
	...
	*/
	if (positionAnchor && win) {
		...
		if (position === "top") offset += box.bottom;
...
function warnIfAnchorIsHidden(positionAnchor, toast) {
	if (positionAnchor.offsetParent === null) printIonWarning("[ion-toast] - The positionAnchor element for ion-toast was found in the DOM, but appears to be hidden. This may lead to unexpected positioning of the toast.", toast);
}
/**
...
var iosEnterAnimation = (baseEl, opts) => {
	...
	const wrapperEl = getElementRoot(baseEl).querySelector(".toast-wrapper");
	...
	switch (position) {
		...
		case "middle":
			...
			wrapperAnimation.fromTo("opacity", .01, 1);
			break;
		default:
...
var iosLeaveAnimation = (baseEl, opts) => {
	...
	switch (position) {
		case "top":
			...
			break;
		case "middle":
			wrapperAnimation.fromTo("opacity", .99, 0);
			break;
		default:
...
var mdLeaveAnimation = (baseEl) => {
	...
	const wrapperAnimation = createAnimation();
	const wrapperEl = getElementRoot(baseEl).querySelector(".toast-wrapper");
	wrapperAnimation.addElement(wrapperEl).fromTo("opacity", .99, 0);
	return baseAnimation.easing("cubic-bezier(.36,.66,.04,1)").duration(300).addAnimation(wrapperAnimation);
};
/**
...
var createSwipeToDismissGesture = (el, toastPosition, onDismiss) => {
	...
	*/
	const STEP_OFFSET = el.position === "middle" ? .5 : 0;
	/**
	...
	*/
	...
	switch (el.position) {
		case "middle":
			...
			*/
			swipeAnimation.progressStart(true, .5);
			break;
		case "top":
			/**
	...
	const onEnd = (detail) => {
		...
		*/
		...
		if (el.position === "middle") {
			...
			*/
			shouldDismiss = threshold >= DISMISS_THRESHOLD / 2 || threshold <= -.5 / 2;
			/**
			...
			*/
			...
			remainingDistance = (shouldDismiss ? 1 - step : step) * MAX_SWIPE_DISTANCE;
		}
		/**
 ...
 /**
				...
				* up or down again.
				*/
				swipeAnimation.keyframes(SWIPE_UP_DOWN_KEYFRAMES).progressStart(true, .5);
				else swipeAnimation.progressStart(true, 0);
				/**
	...
	constructor(hostRef) {
		...
		*/
		this.createSwipeGesture = (toastPosition) => {
			(this.gesture = createSwipeToDismissGesture(this.el, toastPosition, () => {
				/**
 ...
 /**
	...
	*/
	async present() {
		...
		const animationPosition = getAnimationPosition(position, this.getAnchorElement(), getIonMode(this), el);
		/**
		...
		*/
		...
		if (this.duration > 0) this.durationTimeout = setTimeout(() => this.dismiss(void 0, "timeout"), this.duration);
		/**
		* If the Toast has a swipe gesture then we can
		* create the gesture so users can swipe the
		* presented Toast.
		*/
		if (this.prefersSwipeGesture()) this.createSwipeGesture(animationPosition);
		unlock();
	}
	/**
	...
	*/
	...
	getButtons() {
		return this.buttons ? this.buttons.map((b) => {
	...
	*/
	getAnchorElement() {
		...
		*/
		if (positionAnchor === void 0) return;
		...
		if (positionAnchor instanceof HTMLElement) return positionAnchor;
		printIonWarning("[ion-toast] - Invalid positionAnchor value:", positionAnchor, el);
	}
	async buttonClick(button) {
		const role = button.role;
		if (isCancel(role)) return this.dismiss(void 0, role);
		if (await this.callButtonHandler(button)) return this.dismiss(void 0, role);
		return Promise.resolve();
	}
	async callButtonHandler(button) {
		if (button?.handler) try {
			if (await safeCall(button.handler) === false) return false;
		} catch (e) {
	...
	renderButtons(buttons, side) {
		...
		return h("div", { class: {
			...
			part: buttonPart(b)
		}, h("div", { class: "toast-button-inner" }, b.icon && h("ion-icon", {
			"aria-hidden": "true",
			icon: b.icon,
			slot: b.text === void 0 ? "icon-only" : void 0,
			class: "toast-button-icon"
		}), b.text), mode === "md" && h("ion-ripple-effect", { type: b.icon !== void 0 && b.text === void 0 ? "unbounded" : "bounded" }))));
	}
	/**
	...
	*/
	...
	render() {
		...
		*/
		if (layout === "stacked" && startButtons.length > 0 && endButtons.length > 0) printIonWarning("[ion-toast] - This toast is using start and end buttons with the stacked toast layout. We recommend following the best practice of using either start or end buttons with the stacked toast layout.", el);
		return h(Host, {
			key: "15d2ff8b689e40bb5cae3e304448dadf91b11b88",
			tabindex: "-1",
			...this.htmlAttributes,
