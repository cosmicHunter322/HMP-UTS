...
import { C as win, v as raf } from "./helpers-BJFnZngp-COG28xNj.js";
//#region node_modules/@ionic/core/dist/esm/slot-mutation-controller-D7n8Peo2.js
/*!
...
var createNotchController = (el, getNotchSpacerEl, getLabelSlot) => {
	...
	const needsExplicitNotchWidth = () => {
		if (getNotchSpacerEl() === void 0 || el.label !== void 0 || getLabelSlot() === null) return false;
		return true;
	};
 ...
 /**
	...
	*/
	const setNotchWidth = () => {
		...
		if (notchSpacerEl === void 0) return;
		if (!needsExplicitNotchWidth()) {
			notchSpacerEl.style.removeProperty("width");
			return;
		}
		const width = getLabelSlot().scrollWidth;
		if (width === 0 && notchSpacerEl.offsetParent === null && win !== void 0 && "IntersectionObserver" in win) {
			/**
			...
			* another one.
			*/
			if (notchVisibilityIO !== void 0) return;
		...
		*/
		notchSpacerEl.style.setProperty("width", `${width * .75}px`);
	};
	const destroy = () => {
...
var createStartContainerController = (el, getStartContainer, shouldApplyAdjustment) => {
	...
	const calculateAdjustment = () => {
		...
		if (!startSlot || !shouldApplyAdjustment()) return "";
	...
	const scheduleAdjustmentUpdate = () => {
		if (measurementRaf !== void 0) return;
		measurementRaf = raf(() => {
			measurementRaf = void 0;
			const adjustment = calculateAdjustment();
			if (adjustment !== appliedAdjustment) {
				if (transitionRaf !== void 0) cancelAnimationFrame(transitionRaf);
	...
	const setupResizeObserver = () => {
		...
		if (typeof ResizeObserver !== "function" || resizeObserver && observedContainer === startSlot) return;
		disconnectResizeObserver();
...
* click.
*/
var getSlottedClickContent = (ev, el) => {
...
* end slot.
*/
var isSlottedClick = (ev, el) => getSlottedClickContent(ev, el) !== null;
/**
...
*/
var createClickController = (el, getNativeInput) => {
	...
	const handleClickCapture = (ev) => {
		...
		if (nativeInput !== void 0 && ev.target === nativeInput) {
			ev.stopPropagation();
			if (!hasSlottedClick) el.click();
			hasSlottedClick = false;
		}
	};
	...
	*/
	const watchForSlotChange = (slottedEl) => {
		...
		slottedContentMutationObserver = new MutationObserver((entries) => {
			mutationCallback();
			for (const entry of entries) for (const node of entry.removedNodes)
 /**
			* If the element was removed then we
			* need to destroy the MutationObserver
			* so the element can be garbage collected.
			*/
			if (node.nodeType === Node.ELEMENT_NODE && node.slot === slotName) destroySlottedContentObserver();
		});
		/**
		...
		*/
		slottedContentMutationObserver.observe(slottedEl.parentElement ?? slottedEl, {
			subtree: true,
			childList: true
		});
	};
...
//#endregion
export { getSlottedClickContent as a, createStartContainerController as i, createNotchController as n, createSlotMutationController as r, createClickController as t };
